...
function relationshipTypes(xml: string): string[] {
	return elements(xmlDocument(xml), "Relationship").map(
...
describe("office file helpers", () => {
	it("normalizes file names and extensions from paths or refs", () => {
		...
		expect(
			getOfficeFileName({ filename: "memo.docx", path: "ignored.xlsx" }),
		).toBe("memo.docx");
	...
	it("round-trips Univer document plain text snapshots", () => {
		const snapshot = plainTextToUniverDocumentSnapshot(
			"memo.docx",
			"first line\nsecond line",
		);

		expect(snapshot.title).toBe("memo.docx");
		expect(univerDocumentSnapshotToPlainText(snapshot)).toBe(
			"first line\nsecond line",
		);
	});
...
describe("workbookBytesToUniverSnapshot", () => {
	...
	it("edits cells in place while preserving advanced chart, drawing, media and relationship parts", async () => {
		...
		const snapshot = workbookBytesToUniverSnapshot(augmented, {
		...
		expect(await zipText(after, worksheetPath)).toContain(
			'<drawing r:id="rId99"/>',
		);
...
describe("pptxBytesToUniverSlideSnapshot", () => {
	it("imports and writes back editable slide text while preserving the package", async () => {
		...
		const snapshot = await pptxBytesToUniverSlideSnapshot(original, {
			filename: "deck.pptx",
		});
	...
	it("imports PowerPoint-native coordinates, theme fill and a non-text rectangle", async () => {
		...
		const snapshot = await pptxBytesToUniverSlideSnapshot(original, {
			filename: "knowledge-smoke.pptx",
		});
		const pageIds = snapshot.body?.pageOrder ?? [];
		const secondPage = pageIds[1]
			? snapshot.body?.pages[pageIds[1]]
			: undefined;
	...
	it("keeps master, layout, theme, relationship and shape geometry parts unchanged on text save", async () => {
		...
		const firstPage = firstPageId
			? snapshot.body?.pages[firstPageId]
			: undefined;
		const firstText = Object.values(firstPage?.pageElements ?? {}).find(
			(element) => element.type === 2,
		);
		if (firstText?.richText)
			firstText.richText.text = "PowerPoint native round-trip BQ-7429";
		...
		for (const part of invariantParts) {
			expect(await after.file(part)?.async("text")).toBe(
				await before.file(part)?.async("text"),
			);
		}
		const firstSlide = await zipText(after, "ppt/slides/slide1.xml");
		const secondSlide = xmlDocument(
			await zipText(after, "ppt/slides/slide2.xml"),
		);
		const rectangle = elements(secondSlide, "sp").find((shape) =>
			elements(shape, "cNvPr").some(
				(nonVisual) => nonVisual.getAttribute("name") === "矩形 4",
			),
		);
		...
		expect(
			elements(rectangle as Element, "prstGeom")[0]?.getAttribute("prst"),
		).toBe("rect");
		expect(elements(transform as Element, "off")[0]).toMatchObject({});
		expect(elements(transform as Element, "off")[0]?.getAttribute("x")).toBe(
			"1180214",
		);
		expect(elements(transform as Element, "off")[0]?.getAttribute("y")).toBe(
			"1786270",
		);
		expect(elements(transform as Element, "ext")[0]?.getAttribute("cx")).toBe(
			"3732028",
		);
		expect(elements(transform as Element, "ext")[0]?.getAttribute("cy")).toBe(
			"1998921",
		);
	});
});
...
describe("PowerPoint-native PPTX package contract", () => {
	it("contains content types, master, layouts, theme and complete relationship chains", async () => {
		const zip = await JSZip.loadAsync(
			new Uint8Array(readFileSync(POWERPOINT_NATIVE_FIXTURE)),
		);
		const requiredParts = [
			"[Content_Types].xml",
			"_rels/.rels",
		...
		const rootRelationships = relationshipTypes(
			await zipText(zip, "_rels/.rels"),
		);
		expect(rootRelationships).toContain(
			"http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument",
		);

		const presentationRelationships = relationshipTypes(
			await zipText(zip, "ppt/_rels/presentation.xml.rels"),
		);
		...
		expect(
			presentationRelationships.filter((type) => type.endsWith("/slide")),
		).toHaveLength(2);

		const masterRelationships = relationshipTypes(
			await zipText(zip, "ppt/slideMasters/_rels/slideMaster1.xml.rels"),
		);
		...
		for (const slideNumber of [1, 2]) {
			const types = relationshipTypes(
				await zipText(zip, `ppt/slides/_rels/slide${slideNumber}.xml.rels`),
			);
			expect(types).toEqual([
				"http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout",
			]);
		}
		const layoutRelationships = relationshipTypes(
			await zipText(zip, "ppt/slideLayouts/_rels/slideLayout1.xml.rels"),
		);
		expect(layoutRelationships).toEqual([
			"http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideMaster",
		]);
	});
	...
	it("provides non-visual shape properties, positive geometry and a theme-backed rectangle", async () => {
		...
		expect(elements(properties, "prstGeom")[0]?.getAttribute("prst")).toBe(
			"rect",
		);
		expect(
			elements(rectangle as Element, "fillRef")[0]?.getAttribute("idx"),
		).toBe("1");
		expect(elements(rectangle as Element, "fillRef")[0]?.textContent).toBe("");
		expect(
			elements(
				elements(rectangle as Element, "fillRef")[0],
				"schemeClr",
			)[0]?.getAttribute("val"),
		).toBe("accent1");
	});
	...
	it("terminates safely for deterministic truncated and byte-flipped PPTX inputs", async () => {
		...
		for (const [index, input] of mutations.entries()) {
			...
			const outcome = await Promise.race([
				pptxBytesToUniverSlideSnapshot(input, {
					filename: `mutated-${index}.pptx`,
				})
						...
						rows: [
							new TableRow({
								children: [
									new TableCell({
										children: [
											new Paragraph({ children: [new TextRun("Cell A1")] }),
										],
									}),
									new TableCell({
										children: [
											new Paragraph({ children: [new TextRun("Cell B1")] }),
										],
									}),
								],
							}),
							new TableRow({
								children: [
									new TableCell({
										children: [
											new Paragraph({ children: [new TextRun("Cell A2")] }),
										],
									}),
									new TableCell({
										children: [
											new Paragraph({ children: [new TextRun("Cell B2")] }),
										],
									}),
								],
							}),
...
describe("docx rich-text round-trip", () => {
	it("preserves headings, bold/italic runs, list items and table cells through import → export → import", async () => {
		...
		const first = await docxBytesToUniverDocumentSnapshot(original, {
			filename: "report.docx",
		});
		expect(first.title).toBe("report.docx");

		// 标题段落带命名样式（HEADING_1 = 4）。
		const hasHeading = (first.body?.paragraphs ?? []).some(
			(p) => p.paragraphStyle?.namedStyleType === 4,
		);
		...
		// 斜体 run。
		const hasItalicRun = (first.body?.textRuns ?? []).some(
			(r) => r.ts?.it === 1,
		);
		expect(hasItalicRun).toBe(true);

		// 列表项（bullet）。
		const bulletCount = (first.body?.paragraphs ?? []).filter(
			(p) => p.bullet,
		).length;
		...
		const second = await docxBytesToUniverDocumentSnapshot(exported, {
			filename: "report.docx",
		});
		...
		expect(
			(second.body?.paragraphs ?? []).some(
				(p) => p.paragraphStyle?.namedStyleType === 4,
			),
		).toBe(true);
		expect((second.body?.textRuns ?? []).some((r) => r.ts?.bl === 1)).toBe(
			true,
		);
		expect(
			(second.body?.paragraphs ?? []).filter((p) => p.bullet).length,
	...
	it("falls back to plain-text behaviour when rich parsing yields no markup", async () => {
		...
		const doc = new Document({
			sections: [
				{
					children: [new Paragraph({ children: [new TextRun("plain only")] })],
				},
			],
		});
		const bytes = new Uint8Array(await Packer.toArrayBuffer(doc));

		const snapshot = await docxBytesToUniverDocumentSnapshot(bytes, {
			filename: "plain.docx",
		});
	...
	it("rejects legacy .doc binaries explicitly", async () => {
		await expect(
			docxBytesToUniverDocumentSnapshot(new Uint8Array([1, 2, 3]), {
				filename: "legacy.doc",
			}),
	...
	it("patches body text while preserving headers, comments, media, relationships and section references", async () => {
		...
		zip.file(
			"word/media/preserve.png",
			new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]),
		);
		const augmented = await zip.generateAsync({ type: "uint8array" });
		const snapshot = await docxBytesToUniverDocumentSnapshot(augmented, {
			filename: "advanced.docx",
		});
		const baseline = JSON.parse(JSON.stringify(snapshot));
		...
		expect(updatedDocument).toContain(
			'w:headerReference w:type="default" r:id="rId99"',
		);
	});

	it("adds a new body paragraph while preserving advanced DOCX parts", async () => {
		const original = await richDocxBytes();
		const zip = await JSZip.loadAsync(original);
		const documentXml = await zipText(zip, "word/document.xml");
		zip.file(
		...
		zip.file(
			"word/header99.xml",
			'<w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:p><w:r><w:t>keep-header</w:t></w:r></w:p></w:hdr>',
		);
		const augmented = await zip.generateAsync({ type: "uint8array" });
		const snapshot = await docxBytesToUniverDocumentSnapshot(augmented, {
			filename: "advanced.docx",
		...
		const insertionAt =
			body.tables?.[0]?.startIndex ?? body.dataStream.lastIndexOf("\n");
		const inserted = "新增段落\r";
		body.dataStream = `${body.dataStream.slice(0, insertionAt)}${inserted}${body.dataStream.slice(insertionAt)}`;
		body.paragraphs = [
			...(body.paragraphs ?? []).map((paragraph) => ({
				...paragraph,
				startIndex:
					paragraph.startIndex >= insertionAt
						? paragraph.startIndex + inserted.length
						: paragraph.startIndex,
			})),
			{ startIndex: insertionAt + inserted.length - 1 },
		].sort((left, right) => left.startIndex - right.startIndex);
		body.textRuns = (body.textRuns ?? []).map((run) => ({
			...run,
			st: run.st >= insertionAt ? run.st + inserted.length : run.st,
			ed: run.ed >= insertionAt ? run.ed + inserted.length : run.ed,
		}));
		body.tables = (body.tables ?? []).map((table) => ({
			...table,
			startIndex:
				table.startIndex >= insertionAt
					? table.startIndex + inserted.length
					: table.startIndex,
			endIndex:
				table.endIndex >= insertionAt
					? table.endIndex + inserted.length
					: table.endIndex,
		}));
		body.sectionBreaks = (body.sectionBreaks ?? []).map((section) => ({
			...section,
			startIndex:
				section.startIndex >= insertionAt
					? section.startIndex + inserted.length
					: section.startIndex,
		}));
		...
		const after = await JSZip.loadAsync(output);
		expect(await after.file("word/header99.xml")?.async("text")).toContain(
			"keep-header",
		);
		const outputDocumentXml = await after
			.file("word/document.xml")
			?.async("text");
		expect(outputDocumentXml).toContain("新增段落");
		const outputDocument = new DOMParser().parseFromString(
			outputDocumentXml ?? "",
			"application/xml",
		);
		const insertedParagraph = Array.from(
			outputDocument.getElementsByTagNameNS(
				"http://schemas.openxmlformats.org/wordprocessingml/2006/main",
				"p",
			),
		).find((paragraph) => paragraph.textContent?.includes("新增段落"));
		expect((insertedParagraph?.parentNode as Element | null)?.localName).toBe(
			"body",
		);
		const roundTrip = await docxBytesToUniverDocumentSnapshot(output, {
			filename: "advanced.docx",
		});
		expect(dataStreamText(roundTrip)).toContain("新增段落");
	});
