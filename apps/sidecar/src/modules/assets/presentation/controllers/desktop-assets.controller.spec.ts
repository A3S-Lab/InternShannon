...
function createHarness(metadata: Record<string, unknown> = {}) {
    ...
    const curation = new KnowledgeCurationService(service, content, graph, ingestion, knowledge, audit);
...
describe("DesktopAssetsController knowledge safety", () => {
    it("returns a lightweight personal-knowledge bootstrap without repository history", async () => {
        const hugeDiff = "diff".repeat(250_000);
        const { asset, controller } = createHarness({
            knowledge: { personal: true },
            commitDiffs: { "commit-1": hugeDiff },
            commits: Array.from({ length: 100 }, (_, index) => ({ sha: `commit-${index}` })),
            blobContents: { "raw/sources/example.md": "# Example" },
            knowledgeCurationSuggestions: [{ id: "suggestion-1", proposedContent: hugeDiff }],
        });

        const result = await controller.getMyKnowledge(asset.ownerId);
        ...
        expect(result.metadata).not.toHaveProperty("commitDiffs");
        expect(result.metadata).not.toHaveProperty("commits");
        expect(result.metadata).not.toHaveProperty("blobContents");
        expect(result.metadata).not.toHaveProperty("knowledgeCurationSuggestions");
        expect(Buffer.byteLength(JSON.stringify(result), "utf8")).toBeLessThan(50_000);
    });
    ...
    it("omits internal snapshot and trash files from the source catalog", async () => {
        const { asset, controller, service } = createHarness();
        await service.updateBlob(asset.id, "raw/sources/live.md", "# Visible", "Add", "main");
        await service.updateBlob(asset.id, "raw/sources/.shuan-os-snapshots/live.md", "# Snapshot", "Snapshot", "main");
        await service.updateBlob(asset.id, "raw/sources/.shuan-os-trash/live.md", "# Trash", "Trash", "main");

        expect((await controller.listWikiSources(asset.id)).map(source => source.path)).toEqual(["raw/sources/live.md"]);
    });

    it("round-trips uploaded binary source bytes without UTF-8 decoding", async () => {
        const { asset, controller } = createHarness();
        const original = Buffer.from([0x00, 0xff, 0xfe, 0x25, 0x50, 0x44, 0x46, 0x0a]);

        const uploaded = await controller.uploadWikiSources(asset.id, {
            sources: [{ name: "sample.pdf", contentBase64: original.toString("base64") }],
        });
        const stored = await controller.repositoryBlob(asset.id, "raw/sources/sample.pdf");
        ...
        expect(stored.contentSha).toBe(createHash("sha1").update(original).digest("hex"));
    ...
    it("persists desktop source locations and clears stale locations for browser uploads and deletes", async () => {
        const { asset, controller } = createHarness();
        const contentBase64 = Buffer.from("source body").toString("base64");
        await controller.uploadWikiSources(asset.id, {
            sources: [{ name: "source.txt", contentBase64, originalPath: "/Users/test/Documents/source.txt" }],
        });

        expect(await controller.listWikiSources(asset.id)).toEqual([
            expect.objectContaining({
                path: "raw/sources/source.txt",
                originalPath: "/Users/test/Documents/source.txt",
            }),
        ]);

        await controller.uploadWikiSources(asset.id, {
            sources: [{ name: "source.txt", contentBase64 }],
        });
        expect((await controller.listWikiSources(asset.id))[0]?.originalPath).toBeUndefined();

        await controller.uploadWikiSources(asset.id, {
            sources: [{ name: "source.txt", contentBase64, originalPath: "/Users/test/Documents/source.txt" }],
        });
        await controller.deleteWikiSource(asset.id, "raw/sources/source.txt");
        expect(
            ((asset.metadata?.knowledge as Record<string, unknown>)?.sourceLocations as Record<string, string>)[
                "raw/sources/source.txt"
            ],
        ).toBeUndefined();
    });

    it("rejects relative desktop source locations", async () => {
        const { asset, controller } = createHarness();
        await expect(
            controller.uploadWikiSources(asset.id, {
                sources: [
                    {
                        name: "source.txt",
                        contentBase64: Buffer.from("source body").toString("base64"),
                        originalPath: "Documents/source.txt",
                    },
                ],
            }),
		).rejects.toThrow();
    ...
    it("keeps binary encoding metadata through rename and removes it on delete", async () => {
        ...
        await service.updateBlobBinary(asset.id, "raw/sources/a.bin", original, "Import a.bin", "main");

        await service.renameBlob(asset.id, "raw/sources/a.bin", "raw/sources/b.bin", "Rename", "main");
        ...
        await expect(service.getBlobContent(asset.id, "raw/sources/b.bin")).rejects.toThrow(
            "二进制文件不能作为 UTF-8 文本读取",
        );

        await controller.deleteWikiSource(asset.id, "raw/sources/b.bin");
        expect((asset.metadata?.blobEncodings as Record<string, string>)["raw/sources/b.bin"]).toBeUndefined();
        await expect(service.getBlobData(asset.id, "raw/sources/b.bin")).rejects.toThrow();
    });
    ...
    it("accepts base64 repository updates used by asset Office editors", async () => {
        ...
        const stored = await controller.repositoryBlob(asset.id, "raw/sources/report.docx");
    ...
    it("reports resolved links, broken links, and true orphan pages", async () => {
        ...
        expect(health.brokenLinks).toEqual([{ srcPath: "wiki/a.md", srcTitle: "A", target: "Missing" }]);
        expect(health.orphanPages).toEqual([{ path: "wiki/a.md", title: "A", type: "concept" }]);
    ...
    it("imports, validates, and exports an OKF ZIP without rewriting extensions", async () => {
        ...
        archive.file("example/index.md", '---\nokf_version: "0.1"\n---\n\n# Example\n');
        archive.file("example/playbooks/incident.md", concept);
        archive.file("example/metrics/revenue.md", "---\ntype: Metric\ntitle: Revenue\n---\n\n# Revenue\n");
        ...
        const exportedArchive = await JSZip.loadAsync(Buffer.from(exported.contentBase64, "base64"));
        ...
        expect(await service.getBlobContent(asset.id, "wiki/playbooks/incident.md")).toBe(concept);
        expect(await exportedArchive.file("playbooks/incident.md")?.async("string")).toBe(concept);
    ...
    it("rejects ZIP entries that attempt to escape the OKF bundle root", async () => {
        const { asset, controller, service } = createHarness();
        const archive = new JSZip();
        archive.file("concepts/valid.md", "---\ntype: Note\ntitle: Valid\n---\n\n# Valid\n");
        archive.file("../../../outside.md", "---\ntype: Note\ntitle: Outside\n---\n\n# Outside\n");
        ...
        await expect(service.getBlobContent(asset.id, "wiki/concepts/valid.md")).rejects.toThrow();
    });
    ...
    it("keeps semantic link suggestions review-only and audits accept/reject decisions", async () => {
        ...
        const accepted = await controller.reviewCurationSuggestion(asset.id, refreshed.suggestions[0].id, "user-1", {
            decision: "accept",
        });
        const rejected = await controller.reviewCurationSuggestion(asset.id, refreshed.suggestions[1].id, "user-1", {
            decision: "reject",
        });
        ...
        const refreshedAccepted = refreshedAgain.suggestions.find((item) => item.id === accepted.id);
        ...
        const reverted = await controller.reviewCurationSuggestion(asset.id, accepted.id, "user-1", {
            decision: "revert",
        });
    ...
    it("reverts only the exact content applied by an accepted curation suggestion", async () => {
        ...
        const reverted = await controller.reviewCurationSuggestion(asset.id, suggestion.id, "user-1", {
            decision: "revert",
        });
    ...
    it("resumes an interrupted append accept without duplicating content", async () => {
        ...
        const refreshed = await controller.refreshCurationSuggestions(asset.id);
        const suggestion = refreshed.suggestions.find(
            (item) => item.kind === "link" && item.sourcePath === "wiki/a.md",
        );
        ...
        expect(
            (asset.metadata?.knowledgeCurationSuggestions as Array<Record<string, unknown>>).find(
        ...
        expect(updateBlob.mock.calls.filter(([, path]) => path === suggestion?.sourcePath)).toHaveLength(2);
    });
    ...
    it("resumes an interrupted append revert without deleting user content twice", async () => {
        ...
        expect(reverted.status).toBe("reverted");
        expect(await service.getBlobContent(asset.id, accepted.sourcePath)).toBe(original);
        expect(updateBlob.mock.calls.filter(([, path]) => path === accepted.sourcePath)).toHaveLength(2);
    });
    ...
    it("excludes generated and draft pages from curation and drops stale pending recursion", async () => {
        ...
        const refreshed = await controller.refreshCurationSuggestions(asset.id);
        expect(refreshed.suggestions).not.toEqual(
            expect.arrayContaining([expect.objectContaining({ id: stalePending.id })]),
        );
        expect(refreshed.suggestions).toEqual(
            expect.arrayContaining([expect.objectContaining({ id: reviewedHistory.id, status: "rejected" })]),
        );
        expect(
            refreshed.suggestions
                ...
                .every(
                    (item) =>
                        !item.sourcePath.startsWith("wiki/generated/") && !item.sourcePath.startsWith("wiki/drafts/"),
                ),
        ).toBe(true);
    });
    ...
    it("adds source-concept edges, communities, and graph filters from OKF frontmatter", async () => {
        ...
        expect(graph.edges).toEqual([
            expect.objectContaining({
                source: "raw/sources/policy.txt",
                target: "wiki/policy.md",
                kind: "source-concept",
            }),
        ]);
        expect(graph.filters).toEqual(
            expect.objectContaining({ types: expect.arrayContaining(["Policy", "Source"]), tags: ["governance"] }),
        );
    });
});
