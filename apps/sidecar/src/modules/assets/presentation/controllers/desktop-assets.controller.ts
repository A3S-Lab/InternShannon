...
import {
    isInternalKnowledgePath,
    isPublicKnowledgeSourcePath,
} from "@/modules/assets/domain/knowledge/knowledge-source-path.policy";
import {
...
export class DesktopAssetsController {
    ...
    async getMyKnowledge(@DesktopOwnerId() userId: string) {
        return this.personalKnowledgeAssetDto(await this.assets.getOrCreatePersonalKnowledge(userId));
    }
    ...
    async listWikiSources(@Param("id") id: string) {
        ...
        return this.content.sourceEntries(asset).filter((source) => isPublicKnowledgeSourcePath(source.path)).map((source) => {
    ...
    /**
     * Knowledge-page bootstrap only needs asset identity and repository routing.
     * Do not transfer the persisted repository history here: `commitDiffs` can
     * grow to tens of megabytes and is available through dedicated repository
     * endpoints when a caller actually requests it.
     */
    private personalKnowledgeAssetDto(asset: Asset) {
                  ...
                  Object.entries(metadata).filter(
                      ([key]) =>
                          ![
                              ...
                              "commitDiffs",
                              "blobEncodings",
                              "knowledgeCurationSuggestions",
                              "knowledgeCurationAudit",
                          ].includes(key),
                  ),
              )
            : undefined;
        return {
            ...props,
            ...(lightweightMetadata && Object.keys(lightweightMetadata).length > 0
    ...
    private treeItems(asset: Asset, dirPath: string): RepositoryTreeItem[] {
        ...
        for (const blob of this.contentBlobs(asset)) {
            if (isInternalKnowledgePath(blob.path)) continue;
