...
export function describeLockedSessionViolation(payload: {
    ...
    if (typeof payload.model === 'string' && payload.model.trim()) {
        return '资产/知识库智能体必须使用系统内核配置的默认模型，不允许指定 model';
    }
    if (typeof payload.systemPrompt === 'string' && payload.systemPrompt.trim()) {
        return '资产/知识库智能体的系统提示词由后端内置规格提供，不允许指定 systemPrompt';
    }
    if (
        ...
        return '资产/知识库智能体的执行权限由系统固定，不允许指定 permissionMode';
    }
    if (
        ...
        return '资产/知识库智能体强制启用规划模式，不允许指定 planningMode';
    }
    if (payload.goalTracking === false) {
        return '资产/知识库智能体强制启用任务追踪，不允许 goalTracking=false';
    }
    if (payload.followDefaultModel === false) {
        return '资产/知识库智能体必须跟随系统默认模型，不允许 followDefaultModel=false';
    }
    return null;
}
