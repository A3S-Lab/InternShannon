...
import type { KnowledgeSourceReference } from "./knowledge-source-reference";
...
export class KernelSessionSnapshotService {
    ...
    private toHistoryMessage(message: Message): KernelSessionHistoryMessage | null {
        ...
        if (message.role === "assistant") {
            ...
            return {
                type: "assistant",
                runId: this.nonEmptyTrimmedString(message.metadata.parentRunId),
                parentToolUseId: null,
                message: {
                    id: message.id,
                    role: "assistant",
                    ...
                    durationMs:
                        this.normalizeFiniteNumber(message.metadata.durationMs ?? message.metadata.duration_ms) ?? null,
                    meta: null,
                    usage: this.normalizeUsage(message.metadata),
                    knowledgeSources: this.knowledgeSourcesFromMetadata(message.metadata),
                    knowledgeSourceProtocolVersion:
                        message.metadata.knowledgeSourceProtocolVersion === 1 ? 1 : undefined,
                },
                timestamp,
            };
        }
    ...
    private knowledgeSourcesFromMetadata(metadata: Record<string, unknown>): KnowledgeSourceReference[] | undefined {
        if (!Array.isArray(metadata.knowledgeSources)) return undefined;
        const sources = metadata.knowledgeSources.filter((value): value is KnowledgeSourceReference => {
            if (!value || typeof value !== "object" || Array.isArray(value)) return false;
            const source = value as Partial<KnowledgeSourceReference>;
            return (
                ...
                source.resource === `asset://${source.assetId}/${source.relativePath}` &&
