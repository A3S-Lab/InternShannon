...
/**
 * Dev loop — read-only adapter (third of the three loops).
 *
 ...
 */
@Injectable()
export class DevLoopController implements LoopController, LoopSpecProvider, OnModuleInit {
    readonly kind = 'dev' as const;
    readonly laneId = 'prompt' as const; // dev loop is model/LLM-generation paced
    private readonly logger = new Logger(DevLoopController.name);

    constructor(
        // Kept for DI/positional-construction compatibility, but no longer used: this controller
    ...
    loopSpec(): LoopSpec {
        return {
            ...
            trigger: {
                ...
                detail:
                    '由真实用户对话轮次驱动(书小安/OrchestrationAgent),每轮 LLM 生成后将未通过的验证(verify issues)经 extra() 跨轮重注入,直到目标达成。不经 LoopRunDriver / loop_runs 表,也无扫描器入队。',
            },
            scope: {
