...
/// Browser helper script injected into every page
const BROWSER_HELPER_SCRIPT: &str = r#"
(function() {
  if (window.__internshannon_injected) return;
  window.__internshannon_injected = true;
  ...
  // Expose helper APIs for page inspection and interaction
  window.__internshannon = {
...
pub async fn browser_open(
    ...
    .initialization_script(BROWSER_HELPER_SCRIPT)
    .auto_resize();
...
pub async fn browser_get_page_text(app: AppHandle, tab_id: String) -> Result<String, String> {
    ...
    webview
        .eval("window.__internshannon?.getPageText() ?? ''")
        .map_err(|e| format!("eval failed: {e}"))?;
    Ok(String::new())
}
