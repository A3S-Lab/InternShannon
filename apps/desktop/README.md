...
<p align="center">
  ...
  <a href="#desktop-local-quick-start-current-monorepo">Desktop Local</a> •
...
## Desktop Local Quick Start (Current Monorepo)
...
```json
{
  ...
  "channels": {
    ...
    "webchat": {
      "enabled": true,
      "cors_origins": ["http://localhost:29653"],
      "websocket_path": "/ws"
    }
  },
  ...
  "logging": {
    ...
    "audit": {
      "enabled": true,
      "path": "~/.internshannon/logs/audit.log",
      "retention_days": 30
    }
  }
}
...
# Credentials are encrypted and stored in ~/.internshannon/credentials.json
```
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/agent/sessions` | Create agent session. Body: `{"backend?","model?","permission_mode?","cwd?"}`. Returns 201 |
...
| GET | `/api/agent/agent-backends` | List available agent runtimes such as `a3s_code` and `hermes` |
