...
# Post-process bundle (patch libkrun paths, create symlinks, code sign, etc.)
echo "[safeclaw-release] running post-build processing"
if [[ -f scripts/safeclaw-post-build.sh ]]; then
  bash scripts/safeclaw-post-build.sh
elif [[ "$(uname -s)" == "Darwin" && -f scripts/fix-macos-bundle.mjs ]]; then
  # Fallback to old Node.js script if new shell script doesn't exist
  echo "[safeclaw-release] applying macOS bundle fix (legacy)"
...
target_root = Path("src-tauri/target")
platforms = {}

# Detect bundle root: cross-compiled target takes priority over native release
bundle_root = None
target_triple = None
for candidate in sorted(target_root.glob("*/release/bundle")):
    triple = candidate.parent.parent.name
    if triple != "release":  # skip the native release/ dir itself
        bundle_root = candidate
        target_triple = triple
        break

if bundle_root is None and (target_root / "release" / "bundle").exists():
    bundle_root = target_root / "release" / "bundle"

# Directory where latest.json will be written (alongside the installer)
release_root = bundle_root.parent if bundle_root else (target_root / "release")
...
(release_root / "latest.json").write_text(json.dumps(manifest, indent=2) + "\n")
PY

ASSETS=()
while IFS= read -r asset; do
  asset="${asset%$'\r'}"  # strip Windows CRLF trailing CR
...
# Find the release directory: cross-compiled triple takes priority
release_root = None
for candidate in sorted(target_root.glob("*/release/bundle")):
    triple = candidate.parent.parent.name
    if triple != "release":
        release_root = candidate.parent  # <triple>/release
        break
