...
const SIDECAR_ENTRYPOINT = path.join(
	REPO_ROOT,
	"apps",
	"sidecar",
	"dist",
...
Standalone builds require the controlled A3S package pinned by
config/controlled-a3s-package.json. Missing or mismatched artifacts fail closed.
...
function resetSourceSearchBrowserResource() {
	run(process.execPath, ["scripts/stage-search-browser-resource.mjs", "--clean"]);
}
...
function resetSourceSidecarToDistOnly() {
	...
	// A controlled local package is valid only for standalone staging. Do not
	// leak that parent-process override into the final dist-only cleanup.
	run(
		process.execPath,
		["scripts/stage-sidecar-resources.mjs", "--dist-only"],
		{
			env: { INTERNSHANNON_LOCAL_A3S_PACKAGE: "" },
		},
	);
}

export function cleanupStagedResources({
	exitCode,
	shouldResetSourceSidecar,
	shouldResetSourceNodeRuntime,
	shouldResetSourceSearchBrowser,
	resetSidecar = resetSourceSidecarToDistOnly,
	resetNode = resetSourceNodeRuntime,
	resetSearchBrowser = resetSourceSearchBrowserResource,
	logError = (message) => console.error(message),
}) {
	let cleanupExitCode = exitCode;
	const cleanupSteps = [
		[shouldResetSourceSidecar, resetSidecar],
		[shouldResetSourceNodeRuntime, resetNode],
		[shouldResetSourceSearchBrowser, resetSearchBrowser],
	];
	for (const [enabled, cleanup] of cleanupSteps) {
		if (!enabled) continue;
		try {
			cleanup();
		} catch (error) {
			logError(`build-standalone-tauri: ${error.message}`);
			if (cleanupExitCode === 0) {
				cleanupExitCode = Number.isInteger(error.status) ? error.status : 1;
			}
		}
	}
...
function candidateResourcesDirs(tauriArgs, options) {
	...
	candidates.push(
		...collectBundledResourcesDirs(path.join(releaseDir, "bundle")),
	);
...
function resolveResourcesDir(tauriArgs, options) {
	const validCandidates = candidateResourcesDirs(tauriArgs, options).filter(
		hasSidecarEntrypoint,
	);
	...
	const bundledCandidates = validCandidates
		.filter(
			(candidate) =>
				path.resolve(candidate) !== path.resolve(stagedResourcesDir),
		)
		.sort(
...
function canSmokeRuntime(runtimeTarget) {
	const host = hostRuntimeTarget();
	return (
		runtimeTarget.platform === host.platform && runtimeTarget.arch === host.arch
	);
}
...
function main() {
	...
	let shouldResetSourceSearchBrowser = false;
	try {
		...
		if (!canSmokeRuntime(runtimeTarget)) {
			throw new Error(
				[
					"controlled standalone builds must run natively on the target host;",
					`requested ${formatRuntimeTarget(runtimeTarget)} from ${formatRuntimeTarget(hostRuntimeTarget())}`,
				].join(" "),
			);
		}
		...
		if (options.verify || options.smoke) {
			...
			if (options.verify) {
				...
				if (options.release) {
					run(process.execPath, [
						"scripts/verify-search-browser-resource.mjs",
						"--dir",
						resourcesDir,
					]);
				}
			...
			if (options.smoke) {
				if (canSmokeRuntime(runtimeTarget)) {
					run(process.execPath, [
						"scripts/smoke-standalone-sidecar.mjs",
						"--dir",
						resourcesDir,
					]);
					run(process.execPath, [
						"scripts/smoke-unicode-provider-websocket.mjs",
						"--dir",
						resourcesDir,
					...
					run(process.execPath, [
						...
						"--dir",
						resourcesDir,
						"--verify-manual-compact",
					]);
					run(process.execPath, [
						"scripts/smoke-no-auto-model-switch.mjs",
						resourcesDir,
					]);
					run(process.execPath, [
						"scripts/smoke-zhipu-429-websocket.mjs",
						resourcesDir,
					]);
				} else {
					console.warn(
		...
		exitCode = cleanupStagedResources({
			exitCode,
			shouldResetSourceSidecar,
			shouldResetSourceNodeRuntime,
			shouldResetSourceSearchBrowser,
