...
function findResourcesDir(startDir, sidecarDir) {
	const resolved = path.resolve(startDir);
	if (isFile(path.join(resolved, "main.js"))) {
		return path.basename(resolved) === "sidecar"
...
function validateSidecarDir(sidecarDir, resourcesDir, requireStandalone) {
	...
	const nodeModulesStats = externalRequires.hasNodeModules
		...
		: { files: 0, bytes: 0 };
	const nodeRuntimeStats = bundledNode
		? collectFileStats(path.join(resourcesDir, "node"))
		: { files: 0, bytes: 0 };
	if (requireStandalone) {
		if (!bundledNode) {
			issues.push(
				"missing bundled Node.js runtime: expected node/bin/node or node/node.exe in Resources.",
			);
		}
	...
	if (
		requireStandalone &&
		externalRequires.packages.length > 0 &&
		!externalRequires.hasNodeModules
	) {
		issues.push(
			[
	...
	if (requireStandalone && externalRequires.hasNodeModules) {
		const missing = missingExternalRequires(
			sidecarDir,
			externalRequires.specifiers,
		);
		if (missing.length > 0) {
			issues.push(
				[
	...
	if (externalRequires.hasNodeModules) {
		const workspaceSpecifiers = workspaceRuntimeProbeSpecifiers(
			externalRequires.specifiers,
		);
		const runtimePackageProbe = probeRuntimePackages({
			anchorPath: path.join(sidecarDir, "main.js"),
			nodeExecutable: bundledNode ?? process.execPath,
			specifiers: workspaceSpecifiers,
		});
		if (!runtimePackageProbe.ok) {
			issues.push(
				[
					"sidecar workspace packages resolve but cannot be loaded at runtime:",
					...formatRuntimePackageProbeFailures(runtimePackageProbe),
				].join(" "),
			);
		}
	}
