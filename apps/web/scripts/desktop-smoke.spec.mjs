...
test("normalizes gateway URLs pasted from API endpoints", () => {
	assert.equal(
		normalizeGatewayUrl(" http://127.0.0.1:29653 "),
		"http://127.0.0.1:29653",
	);
	assert.equal(
		normalizeGatewayUrl("http://127.0.0.1:29653/api/v1"),
		"http://127.0.0.1:29653",
	);
	assert.equal(
		normalizeGatewayUrl("http://127.0.0.1:29653/api/v1/health/"),
		"http://127.0.0.1:29653",
	);
});
...
test("fingerprints non-desktop shell HTML", () => {
	const html =
		'<html><head><title>Other app</title></head><body><main id="app"></main></body></html>';
...
test("joins smoke workspace paths with platform-appropriate separators", () => {
	assert.equal(
		joinSmokeWorkspacePath("/tmp/root/", "/users/", "local", "skills"),
		"/tmp/root/users/local/skills",
	);
	assert.equal(
		joinSmokeWorkspacePath(
...
test("requires a user message run to reach a visible socket outcome", () => {
	...
	const diagnosticAssistant = {
		...
		message: {
			...
			message: {
				role: "assistant",
				content: [
					{ type: "text", text: "模型未返回有效响应，请检查系统 AI 配置" },
				],
			},
		},
	};
	...
	assert.doesNotThrow(() =>
		assertSocketRunOutcome("user_message", [
			...intakeOnlyEvents,
			diagnosticAssistant,
		]),
	);
	...
	const guardedResponse = buildSmokeToolConfirmationResponse(
		"session-a",
		{
			...
			toolInput: {
				...
				content:
					"Documentation may mention rm -rf as plain text and must not be treated as a command.",
			},
		},
		{
...
test("detects HITL timeout regressions in smoke events and sidecar logs", () => {
	...
	assert.doesNotThrow(() =>
		assertNoForbiddenHitlFailure("clean", cleanEvents, "", "session-a"),
	);

	assert.deepEqual(
		collectForbiddenHitlDiagnostics(
			cleanEvents,
			"[kernel.tool.confirmation_not_found] sessionId=session-a",
		),
	...
	assert.deepEqual(
		collectForbiddenHitlDiagnostics(
			cleanEvents,
			"Session 'session-a' already has an active operation",
		),
		["already has an active operation"],
	);
});

test("requires one-shot HITL approval to enter controlled A3S tool execution exactly once", () => {
	...
	assert.doesNotThrow(() =>
		assertHitlExecutionLifecycle("hitl", lifecycleEvents),
	);
	assert.throws(
		() =>
			assertHitlExecutionLifecycle("hitl", [
				...lifecycleEvents,
				lifecycleEvents[0],
			]),
		/duplicate tool_confirmation_request/,
	);
	assert.throws(
		() => assertHitlExecutionLifecycle("hitl", lifecycleEvents.slice(0, 1)),
		/never forwarded controlled A3S tool_execution_start/,
	);
});
