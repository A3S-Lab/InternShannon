...
const configuredGatewayUrl =
	process.env.PUBLIC_DESKTOP_GATEWAY_URL || process.env.DESKTOP_GATEWAY_URL;
const configuredWebUrl =
	process.env.PUBLIC_DESKTOP_URL || process.env.DESKTOP_WEB_URL;
const sessionTitle = `Desktop smoke ${new Date().toISOString()}`;
const USER_MESSAGE_OUTCOME_TIMEOUT_MS = positiveIntegerFromEnv(
	"DESKTOP_SMOKE_USER_MESSAGE_TIMEOUT_MS",
	45_000,
);
const FORBIDDEN_HITL_LOG_PATTERNS = [
	...
	/cancelled while waiting in the queue/i,
];
...
async function main() {
	...
	await expectData(
		"agents",
		`${apiBase}/open/kernel/agents?limit=1`,
	...
	await expectData(
		...
		(data) =>
			data?.sessionId === createdSessionId || data?.id === createdSessionId,
	);
	await expectData(
		"session list",
		...
		(data) =>
			...
			data.items.some(
				(item) =>
					item.sessionId === createdSessionId || item.id === createdSessionId,
			),
	);
...
async function expectSocketMessaging(sessionId) {
	...
	socket.on("subscribed", (message) =>
		events.push({ type: "subscribed", message }),
	);
	...
	socket.on("tool_confirmation_request", (request) => {
		...
		try {
			respondToSmokeToolConfirmation(
				socket,
				sessionId,
				request,
			...
			events.push({
				type: "tool_confirmation_response_error",
				message: formatError(error),
			});
		}
	});
	socket.on("connect_error", (error) =>
		events.push({ type: "connect_error", message: error.message }),
	);

	try {
		await waitForEvent(
			events,
			(event) => event.type === "subscribed",
			"socket subscribed",
		);
		...
		await waitForEvent(
			events,
			(event) =>
				event.type === "message" && event.message?.type === "session_status",
			"socket session_status",
		);

		const content = `desktop smoke user message ${Date.now()}`;
		await runSocketUserMessage(
			socket,
			events,
			sessionId,
			content,
		...
		await expectReadOnlyQueryNoHitl(
			socket,
			events,
			sessionId,
			sidecarLogCursor,
		...
		await runSocketUserMessage(
			...
			events,
			sessionId,
			"授权工具已执行完毕。请不要调用工具，只回复“后续会话正常”。",
			"post-hitl-immediate-follow-up",
		);
		...
		await waitForEvent(
			events,
			(event) =>
				event.type === "message" && event.message?.type === "cancelled",
			"socket cancel",
		);
		...
		await waitForEvent(
			...
			(event) =>
				event.type === "message" &&
				event.message?.type === "command_response" &&
				event.message?.command === "/clear",
...
async function runSocketUserMessage(socket, events, sessionId, content, label) {
	...
	try {
		await waitForEvent(
			events,
			(event) =>
				event.type === "message" && isSocketRunTerminalMessage(event.message),
...
async function expectReadOnlyQueryNoHitl(
	socket,
	events,
	sessionId,
	sidecarLogCursor,
	...
	const content =
		"请只查看当前工作目录并用一句话总结，不要写入或修改任何文件。";
	const runEvents = await runSocketUserMessage(
		socket,
		events,
		sessionId,
		content,
...
async function expectWriteHitlResolves(
	socket,
	events,
	sessionId,
	sidecarLogCursor,
	...
	let runEvents;
	try {
		runEvents = await runSocketUserMessage(
			socket,
			events,
			sessionId,
			content,
	...
	assertHitlExecutionLifecycle("write-hitl-resolves", runEvents);
	assertNoForbiddenHitlFailure(
		"write-hitl-resolves",
		runEvents,
		readSidecarLogSince(sidecarLogCursor),
	...
	log(
		`write-hitl-resolves ok (${confirmationRequests.length} confirmation request(s))`,
	);
}

export function assertHitlExecutionLifecycle(label, events) {
	const confirmationRequests = findToolConfirmationRequests(events);
	const requestIds = confirmationRequests
		.map((event) => event?.request?.requestId)
		.filter(Boolean);
	assert(
		new Set(requestIds).size === requestIds.length,
		`${label} emitted a duplicate tool_confirmation_request: ${formatSmokeJson(confirmationRequests, 2000)}`,
	);
	...
	const executionStarts = events.filter(
		(event) =>
			event?.type === "message" &&
			event.message?.type === "stream_event" &&
			event.message?.event?.type === "tool_execution_start",
	);
	assert(
		executionStarts.length > 0,
		`${label} never forwarded controlled A3S tool_execution_start: ${formatSmokeJson(events, 3000)}`,
	);
}
...
function respondToSmokeToolConfirmation(socket, sessionId, request, guard) {
	const response = buildSmokeToolConfirmationResponse(
		sessionId,
		request,
		guard,
	);
	...
	log(
		`${response.approved ? "auto-approved" : "auto-denied"} smoke tool confirmation (${response.toolName})`,
	);
	return response;
}

export function buildSmokeToolConfirmationResponse(
	sessionId,
	request,
	guard = null,
) {
	...
	assert(
		...
		`tool_confirmation_request missing requestId: ${formatSmokeJson(request, 500)}`,
	);
	assert(
	...
	assert(
		...
		`tool_confirmation_request missing toolName: ${formatSmokeJson(request, 500)}`,
	);
...
export function isSmokeToolConfirmationSafe(request, guard = null) {
	...
	const toolName =
		typeof request?.toolName === "string"
			? request.toolName.trim().toLowerCase()
			: "";
	const allowedToolNames = Array.isArray(guard.allowedToolNames)
		? guard.allowedToolNames
		: [];
	if (
		allowedToolNames.length > 0 &&
		!allowedToolNames.some((allowed) =>
			toolName.includes(String(allowed).trim().toLowerCase()),
		)
	) {
		return false;
	}
	...
	if (
		fields.some(
			(field) => isCommandField(field.key) && hasDangerousCommand(field.value),
		)
	) {
		return false;
	}

	const allowedPathFragments = Array.isArray(guard.allowedPathFragments)
		? guard.allowedPathFragments
		: [];
	if (allowedPathFragments.length > 0) {
		const pathFields = fields.filter((field) => isPathField(field.key));
		return pathFields.some((field) =>
			allowedPathFragments.some((fragment) =>
				field.value.includes(String(fragment)),
			),
		);
	}
...
function isPathField(key) {
	return /(^|\.)(path|file_path|filepath|fileName|filename|targetPath|target_path)$/i.test(
		key,
	);
}
...
function hasDangerousCommand(value) {
	return (
		/\brm\s+-rf\b/i.test(value) ||
		/\bsudo\b/i.test(value) ||
		/\bchmod\b/i.test(value)
	);
}

export function assertNoForbiddenHitlFailure(
	label,
	events,
	sidecarLogText = "",
	sessionId = "unknown",
...
export function buildHitlSmokeDiagnostics(
	label,
	sessionId,
	events,
	sidecarLogText = "",
	...
	const confirmationEvents = events
		.filter((event) => event?.type === "tool_confirmation_request")
		.slice(-10);
	const forbiddenLogLines = sidecarLogText
		.split(/\r?\n/)
		.filter((line) =>
			FORBIDDEN_HITL_LOG_PATTERNS.some((pattern) => pattern.test(line)),
		)
		.slice(-20);
	...
	return [
		...
		forbiddenLogLines.length > 0
			? `sidecar forbidden log lines=${forbiddenLogLines.join("\n")}`
			: "",
	]
		.filter(Boolean)
		.join("\n");
...
async function expectSocketMessageHistoryReplay(sessionId, userContent) {
	...
	try {
		await waitForEvent(
			events,
			(event) => event.type === "subscribed",
			"socket replay subscribed",
		);
		const history = await waitForEvent(
			events,
			(event) =>
				event.type === "message" && event.message?.type === "message_history",
...
export function summarizeSocketRunEvidence({
	label,
	sessionId,
	prompt,
	sentAt,
	...
	return {
		...
		terminalFrameTypes: messages
			.filter(isSocketRunTerminalMessage)
			.map((message) => message.type),
		confirmationRequestCount: events.filter(
			(event) => event?.type === "tool_confirmation_request",
		).length,
		streamEventTypes,
	};
}
...
export function assertSessionSnapshotContainsRun(label, snapshot, userContent) {
	...
	const userIndex = messages.findIndex(
		(message) =>
			message?.type === "user_message" && message.content === userContent,
	);
	assert(
		userIndex >= 0,
		`${label} missing smoke user message: ${formatSmokeJson(snapshot, 1000)}`,
	);

	const assistant = messages
		.slice(userIndex + 1)
		.find(isReplayableAssistantMessage);
	assert(
		assistant,
...
export function assertSocketMessageHistoryContainsRun(
	label,
	historyMessage,
	userContent,
) {
	...
	assertSessionSnapshotContainsRun(
		label,
		{ messages: historyMessage.messages },
		userContent,
	);
}
...
async function waitForEvent(events, predicate, label, options = {}) {
	...
	throw new Error(
		`${label} timed out: ${JSON.stringify(events).slice(0, 1000)}`,
	);
}
...
async function cleanup() {
	...
	try {
		...
		console.warn(
			`[desktop-smoke] failed to delete session ${createdSessionId}: ${formatError(error)}`,
		);
	}
}
	...
	for (let port = 29653; port <= 29703; port += 1) {
		...
		try {
			...
			recordProbeAttempt(
				attempts,
				candidate,
				`health payload was not ok: ${JSON.stringify(parsed).slice(0, 160)}`,
			);
		} catch (error) {
		...
		try {
			...
			if (!response.ok) {
				recordProbeAttempt(
					attempts,
					candidate,
					`root returned HTTP ${response.status}`,
				);
				continue;
			}
			const html = await response.text();
			if (
				html.includes("<title>书小安</title>") ||
				html.includes("internshannon-bootstrap")
			) {
				return candidate;
			}
			if (html.trim()) {
				recordProbeAttempt(
					attempts,
					candidate,
					"root responded but did not look like the desktop shell",
				);
			}
		} catch (error) {
			recordProbeAttempt(attempts, candidate, describeProbeError(error));
...
export function assertDesktopShellHtml(label, response, html) {
	assert(
		response.ok,
		`${label} returned HTTP ${response.status}: ${html.slice(0, 500)}`,
	);
	const missing = [
...
function collectInitialAssets(html, webUrl) {
	...
	for (const tag of html.matchAll(/<link\b[^>]*>/gi)) {
		...
		if (relTokens.has("stylesheet")) {
			...
			(asType === "script" || asType === "style")
		) {
			addAsset(
				assets,
				seen,
				asType === "style" ? "style" : "script",
				href,
				webUrl,
			);
		}
...
async function expectAsset(asset) {
	...
	assert(
		response.ok,
		`${asset.kind} asset ${asset.url} returned HTTP ${response.status}: ${text.slice(0, 300)}`,
	);
	assert(
		text.trim().length > 0,
		`${asset.kind} asset ${asset.url} returned an empty response`,
	);
	assert(
		!looksLikeHtmlDocument(text),
		`${asset.kind} asset ${asset.url} returned HTML instead of an asset`,
	);
	return text;
}
...
async function fetchForSmoke(label, url, init) {
	try {
		...
		throw new Error(
			`${label} request failed for ${url}: ${describeProbeError(error)}`,
		);
	}
}
...
async function expectData(label, url, predicate) {
	...
	assert(
		predicate(data),
		`${label} returned unexpected data: ${JSON.stringify(data).slice(0, 500)}`,
	);
	log(`${label} ok`);
}

async function expectWorkspacePrerequisites() {
	const defaultRoot = await requestJson(
		"workspace default root",
		`${apiBase}/workspace/default-root`,
	);
	assert(
	...
	const readiness = await requestJson(
		"workspace readiness repair",
		`${apiBase}/workspace/readiness`,
		{
			method: "POST",
		},
	);
	...
	log(
		`workspace prerequisites ok (root=${readiness.workspaceRoot}, skill dirs=${skillRoots.length})`,
	);
	return {
		skillRoots,
...
function assertWorkspaceReady(label, readiness) {
	assert(
		typeof readiness?.workspaceRoot === "string" &&
			readiness.workspaceRoot.trim().length > 0,
		`${label} did not return workspaceRoot: ${JSON.stringify(readiness).slice(0, 500)}`,
	);
	assert(
		readiness.rootExists === true &&
			readiness.agentsExists === true &&
			readiness.sessionsExists === true,
		`${label} returned incomplete readiness: ${JSON.stringify(readiness).slice(0, 500)}`,
	);
	assert(
		readiness.needsRepair === false,
		`${label} still needs repair: ${JSON.stringify(readiness).slice(0, 500)}`,
	);
}
...
export function assertConfigCategoryList(label, categories) {
	...
	const names = new Set(
		categories.items.map((item) => item?.name).filter(Boolean),
	);
	const missing = ["general", "llm", "search", "storage"].filter(
		(name) => !names.has(name),
	);
	assert(
		missing.length === 0,
		`${label} missing category names: ${missing.join(", ")}`,
	);
}
...
async function expectSkillsWorkspaceRoundTrip(skillRoot) {
	...
	try {
		...
		assert(
			...
			"skills workspace read returned different content",
		);
		...
		await fetchForSmoke(
			...
			`${apiBase}/workspace/delete?path=${encodeURIComponent(path)}`,
			{
				method: "DELETE",
			},
		).catch(() => null);
...
export function assertSkillsDirectoryContains(label, entries, filename) {
	assert(
		...
		`${label} did not return an array: ${JSON.stringify(entries).slice(0, 500)}`,
	);
	assert(
...
export function formatProbeAttempts(attempts) {
	...
	const shown = attempts
		.slice(0, 8)
		.map((attempt) => `${attempt.url}: ${attempt.detail}`);
	const suffix =
		attempts.length > shown.length
			? `; ... ${attempts.length - shown.length} more`
			: "";
	return [
...
export function describeProbeError(error) {
	...
	if (cause && typeof cause === "object") {
		...
		const details = ["code", "syscall", "address", "port"]
			.map((key) =>
				key in causeRecord ? `${key}=${String(causeRecord[key])}` : null,
			)
			.filter(Boolean);
		if (details.length > 0) {
	...
	if (parts.length === 1 && parts[0] === "fetch failed") {
		parts.push(
			"no low-level cause exposed; verify the service is listening and this sandbox can access 127.0.0.1",
		);
	}
