...
async function ensureWasm(): Promise<any> {
	...
	if (!initPromise) {
		initPromise = (async () => {
			try {
				const wasmPath = workspaceAssetPath("wasm/hash/sidex_hash_wasm.js");
