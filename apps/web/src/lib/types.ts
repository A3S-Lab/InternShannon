...
import type { KnowledgeSourceReference } from "./knowledge-citation";
...
export interface AgentSessionState {
	...
	/** Prompt tokens in the latest completed model request. */
	contextUsedTokens?: number;
	/** Effective application model limit also passed to A3S auto-compaction. */
	contextMaxTokens?: number;
	/** True when A3S estimated the next prompt instead of using provider usage. */
	contextUsageEstimated?: boolean;
	/** True after compaction until a subsequent model request refreshes usage. */
	contextUsagePendingRefresh?: boolean;
	isCompacting: boolean;
	/** Authoritative backend single-flight state for the current session. */
	runtimeBusy?: boolean;
	/** Correlates terminal events so a stale run cannot clear a newer run. */
	activeRunId?: string;
	runtimePhase?: string;
...
export interface AgentChatMessage {
	...
	source?: string;
	/** Verified, per-answer sources issued by the sidecar; never inferred from model prose. */
...
export type BrowserIncomingMessage =
	...
	| { type: "cancelled"; cancelled: boolean; runId?: string }
	| {
			type: "tool_progress";
			toolUseId: string;
			toolName: string;
	...
	| { type: "status_change"; status: string | null; runId?: string }
	| {
			type: "auth_status";
...
export type BrowserOutgoingMessage =
	...
	| { type: "session_status"; requestId?: string; contextOnly?: boolean }
	| { type: "clear_session" }
