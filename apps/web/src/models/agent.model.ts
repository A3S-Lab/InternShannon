...
interface AgentStoreState {
	...
	sessionFiles: Record<
		string,
		Array<{ uploadId: string; fileName: string; mimeType?: string }>
	>;
...
const STORAGE_KEY_HIDDEN_INTERNAL_SESSIONS =
	"internshannon-hidden-internal-sessions";
...
function loadHiddenInternalSessions(): Record<string, true> {
	try {
		const sessionIds = readUserJsonStorage<string[]>(
			STORAGE_KEY_HIDDEN_INTERNAL_SESSIONS,
			[],
		);
...
function persistHiddenInternalSessions() {
	writeUserJsonStorage(
		STORAGE_KEY_HIDDEN_INTERNAL_SESSIONS,
		Object.keys(state.hiddenInternalSessions),
	);
}

function loadSdkSessions(): AgentProcessInfo[] {
	try {
		return normalizePersistedSdkSessions(
			readUserJsonStorage<unknown>(STORAGE_KEY_SDK_SESSIONS, []),
			{
				exposeWorkspacePath,
			},
		);
...
function persistSdkSessions(sessions: AgentProcessInfo[]) {
	writeUserJsonStorage(
		STORAGE_KEY_SDK_SESSIONS,
		sessions.map(sanitizeSdkSession),
	);
}
...
function loadPersistedAssistantBlocks(): PersistedAssistantBlocks {
	try {
		const parsed = readUserJsonStorage<PersistedAssistantBlocks>(
			STORAGE_KEY_ASSISTANT_BLOCKS,
			{},
		);
...
function rebuildAssistantBlocksForSession(
	sessionId: string,
...
const state = proxy<AgentStoreState>({
	sessions: Object.fromEntries(
		initialSdkSessions.map((session) => [
			session.sessionId,
			sessionStateFromProcess(session),
		]),
	),
	...
	cliConnected: {},
	sessionStatus: {},
	sessionNames: normalizePersistedSessionNames(
		readUserJsonStorage<unknown>(STORAGE_KEY_NAMES, {}),
	),
	unreadCounts: {},
	pendingChatPrefill: {},
	latestAssetProposal: {},
...
function reloadUserScopedAgentState() {
	...
	state.cliConnected = {};
	state.sessionStatus = {};
	state.sessionNames = normalizePersistedSessionNames(
		readUserJsonStorage<unknown>(STORAGE_KEY_NAMES, {}),
	);
	state.unreadCounts = {};
...
const actions = {
	...
	updateSession(sessionId: string, updates: Partial<AgentSessionState>) {
		...
		if (existing) {
			Object.assign(
				existing,
				"cwd" in updates
					? { ...updates, cwd: exposeWorkspacePath(updates.cwd) }
					: updates,
			);
		}
	},
	...
	onStreamEvent(sessionId: string, listener: StreamEventListener) {
		const listeners =
			streamEventListeners.get(sessionId) ?? new Set<StreamEventListener>();
	...
	addSessionFile(
		sessionId: string,
		file: { uploadId: string; fileName: string; mimeType?: string },
	) {
	...
	addRuntimeTimelineEvent(sessionId: string, event: AgentRuntimeTimelineEvent) {
		...
		const next =
			existingIndex >= 0
				? current.map((item, index) => (index === existingIndex ? event : item))
				: [...current, event];
	...
	setPlanningState(
		sessionId: string,
		planningState: AgentPlanningState | null,
	) {
		if (!planningState) {
	...
	removeSession(sessionId: string) {
		...
		delete state.planningStates[sessionId];
		streamEventListeners.delete(sessionId);
		state.sdkSessions = state.sdkSessions.filter(
	...
	upsertSdkSession(session: AgentProcessInfo) {
		...
		const next = state.sdkSessions.filter(
			(item) => item.sessionId !== session.sessionId,
		);
	...
	removeMessage(sessionId: string, messageId: string) {
		...
		state.messages[sessionId] = next;
		rebuildAssistantBlocksForSession(sessionId, next);
		persistAssistantBlocks();
	},
	...
	updateAgentMessage(
		...
		const msg = state.agentMessages[sessionId]?.find(
	...
	getToolProgress(
		...
		if (toolName) {
			const matches = Object.values(map).filter(
				(progress) => progress.toolName === toolName,
			);
	...
	removeToolProgress(sessionId: string, toolUseId?: string | null) {
		...
		const remaining = state.activeToolProgressById[sessionId]
			...
			: [];
		state.activeToolProgress[sessionId] =
	...
	appendStreamingText(sessionId: string, text: string, seq: number) {
		...
		if (existingTextIdx !== -1) {
			...
			if (
				text.startsWith(existing.content) ||
				existing.content.startsWith(text)
			) {
		...
		if (progress.toolUseId) {
			return segs.findIndex(
				(seg) =>
					...
					seg.progress.toolUseId === progress.toolUseId,
			);
		}
		const sameName = segs
			.map((seg, idx) =>
				seg.type === "tool_progress" &&
				seg.progress.toolName === progress.toolName
					? idx
					: -1,
			)
			.filter((idx) => idx >= 0);
	...
	findCompletedToolSegmentIndex(
		...
		if (call.toolUseId) {
			return segs.findIndex(
				(seg) => seg.type === "tool" && seg.call.toolUseId === call.toolUseId,
			);
		}
		const sameName = segs
			.map((seg, idx) =>
				seg.type === "tool" && seg.call.toolName === call.toolName ? idx : -1,
			)
			.filter((idx) => idx >= 0);
		return sameName.length === 1 ? sameName[0] : -1;
	},
	...
	replaceStreamingToolProgressWithCompleted(
		...
		const segs = state.streamingSegments[sessionId];
		const existingCompletedIdx = actions.findCompletedToolSegmentIndex(
			segs,
			call,
		);
		...
		if (idx >= 0) {
			...
			for (let i = segs.length - 1; i >= 0; i--) {
				...
				if (
					...
					segment?.type === "tool_progress" &&
					(segment.progress.toolUseId === toolUseId ||
						segment.progress.toolName === call.toolName)
				) {
					segs.splice(i, 1);
				}
			}
	...
	prefillChatInput(
		sessionId: string,
		text: string,
		options: AgentChatInputPrefillOptions = false,
	) {
		const autoSend =
			typeof options === "boolean" ? options : options.autoSend === true;
		...
		state.pendingChatPrefill[sessionId] =
			images.length > 0 ? { text, autoSend, images } : { text, autoSend };
	},
	...
	setLatestAssetProposal(
		sessionId: string,
		payload: AgentStoreState["latestAssetProposal"][string],
	) {
		if (!payload) {
			delete state.latestAssetProposal[sessionId];
			return;
	...
	updateBypassConversation(
		sessionId: string,
		turnId: string,
		updates: Partial<BypassConversationTurn>,
	) {
		const turns = state.bypassConversations[sessionId];
		if (!turns) return;
		state.bypassConversations[sessionId] = turns.map((turn) =>
	...
	removeBypassConversation(sessionId: string, turnId: string) {
		const turns = state.bypassConversations[sessionId];
		if (!turns) return;
		state.bypassConversations[sessionId] = turns.filter(
			(turn) => turn.id !== turnId,
		);
	},

	getPersistedAssistantBlocks(
		sessionId: string,
