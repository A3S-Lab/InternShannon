...
export function serializeDefaultModelRef(
	defaultProvider: string,
	defaultModel: string,
): string | undefined {
...
export function normalizeBackendModelConfig(value: {
	...
	const unresolvedQualifiedDefault = defaultRef
		? splitProviderModelRef(defaultRef)
		: null;
	...
	return {
		...
		defaultProvider:
			...
			unresolvedQualifiedDefault?.providerName ??
			"",
		// Keep an unresolved persisted value visible for diagnosis and a future
		// explicit user correction.  Never replace it with providers[0]/models[0].
		defaultModel:
			resolvedDefault?.model ??
			unresolvedQualifiedDefault?.modelId ??
			defaultRef ??
			"",
	};
}
...
function normalizeModels(value: unknown): ModelConfig[] {
	...
	return value.flatMap((item): ModelConfig[] => {
		...
		const toolCall = normalizeOptionalBoolean(
			record.toolCall ?? record.tool_call,
		);
		const temperature = normalizeOptionalBoolean(record.temperature);
		const releaseDate = normalizeDisplayText(
...
function resolveDefaultModelRef(
	...
	const matches = providers.filter((provider) =>
		provider.models.some((model) => model.id === rawRef),
	);
	return matches.length === 1
		? { provider: matches[0].name, model: rawRef }
		: null;
}

function normalizeModalities(
	value: unknown,
): ModelConfig["modalities"] | undefined {
...
function normalizeCost(value: unknown): ModelConfig["cost"] | undefined {
	...
	const cacheRead = normalizeFiniteNumber(
		record.cacheRead ?? record.cache_read,
	);
	const cacheWrite = normalizeFiniteNumber(
		record.cacheWrite ?? record.cache_write,
	);
