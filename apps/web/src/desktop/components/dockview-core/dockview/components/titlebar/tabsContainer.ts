		...
		this.addDisposables(
			...
			addDisposableListener(
				...
				(event) => {
					...
					const maxScrollLeft =
						this.tabContainer.scrollWidth - this.tabContainer.clientWidth;
					if (maxScrollLeft <= 0) {
						return;
					}
					...
					const nextScrollLeft = Math.min(
						Math.max(this.tabContainer.scrollLeft + delta, 0),
						maxScrollLeft,
					);
					if (nextScrollLeft === this.tabContainer.scrollLeft) {
						return;
					}

					event.preventDefault();
					this.tabContainer.scrollLeft = nextScrollLeft;
				},
				{ passive: false },
			),
		);
