...
import { peekKnowledgeReturnAnchor } from "@/components/agent-page/chat/knowledge-return-anchor";
...
import {
	...
	ExplorerHeader,
	OverviewPane,
} from "./components/knowledge-explorer-pane";
import { GraphPane } from "./components/knowledge-graph-pane";
import {
	ingestProgress,
	OperationsPane,
} from "./components/knowledge-operations-pane";
...
export default function KnowledgePage() {
	...
	const returnAnchor = peekKnowledgeReturnAnchor();
	const canReturnToChat =
		(location.state as { fromChatSource?: unknown } | null)?.fromChatSource ===
			true || Boolean(returnAnchor);
	const fromChatSource =
		(location.state as { fromChatSource?: unknown } | null)?.fromChatSource ===
		true;
	...
	const [curationSuggestions, setCurationSuggestions] = useState<
		WikiCurationSuggestion[]
	>([]);
	...
	const [knowledgeConfig, setKnowledgeConfig] = useState<WikiConfig | null>(
		null,
	);
	...
	const [editorState, setEditorState] =
		useState<AssetFileManagerStateSnapshot | null>(null);
	...
	const activeRelativeFile = useMemo(
		() => relativeActiveFile(editorState?.activeFile),
	...
	const activeIngestProgress = activeIngestJob
		? ingestProgress(activeIngestJob)
		: null;
	const nativeRevealPaths = useMemo(() => {
		...
		return Object.fromEntries(
			sources
				.filter(
					(source): source is WikiSourceEntry & { originalPath: string } =>
	...
	const citationOpenRequest = useMemo(() => {
		...
		const belongsToPersonalKnowledge =
			...
			citation.assetId === "personal-knowledge" ||
			citation.assetId === asset.name;
		if (!belongsToPersonalKnowledge) return null;
		return {
			path: `${assetRoot}/${citation.relativePath}`,
			nonce: openRequest,
			preview: fromChatSource,
			pinned: !fromChatSource,
		};
	}, [asset, assetRoot, fromChatSource, searchParams]);

	const handleReturnToChat = useCallback(() => {
		// A source preview can be opened while the previous history entry is the
		// regular knowledge page. Browser back would then return to knowledge rather
		// than the originating chat, so this action always targets chat explicitly.
		navigate("/", {
			replace: true,
			state: returnAnchor
				? {
						knowledgeReturnRequestId: returnAnchor.requestId,
						knowledgeReturnAnchor: returnAnchor,
					}
				: undefined,
		});
	}, [navigate, returnAnchor]);

	const loadKnowledge = useCallback(async () => {
		setLoadState("loading");
		setError(null);
		try {
			const nextAsset = await assetsApi.getMyKnowledge({ timeoutMs: 15_000 });
			setAsset(nextAsset);
			// Asset identity is sufficient to render and open the repository. Keep
			// secondary dashboard metadata from blocking the entire knowledge page.
			setLoadState("ready");
			const [
				nextHealth,
				nextSources,
				...
				assetsApi
					.wikiHealth(nextAsset.id, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => null),
				assetsApi
					.wikiListSources(nextAsset.id, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => []),
				assetsApi
					.wikiListPages(nextAsset.id, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => []),
				assetsApi
					.wikiGraph(nextAsset.id, undefined, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => null),
				assetsApi
					.wikiListCurationSuggestions(nextAsset.id, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => ({ assetId: nextAsset.id, suggestions: [] })),
				assetsApi
					.wikiListIngestJobs(nextAsset.id, 20, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => []),
				assetsApi
					.wikiAuditLog(nextAsset.id, 50, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => []),
				assetsApi
					.wikiGetConfig(nextAsset.id, {
						timeoutMs: 10_000,
						suppressErrorToast: true,
					})
					.catch(() => null),
	...
	const refreshMetadata = useCallback(async () => {
		...
		const [
			...
			assetsApi
				.wikiHealth(asset.id, { suppressErrorToast: true })
				.catch(() => null),
			assetsApi.wikiListSources(asset.id).catch(() => []),
			assetsApi
				.wikiListPages(asset.id, { suppressErrorToast: true })
				.catch(() => []),
	...
	const handleReviewCuration = useCallback(
		async (suggestionId: string, decision: "accept" | "reject" | "revert") => {
			if (!asset) return;
			const suggestion = curationSuggestions.find(
			...
			setBusy(true);
			try {
				await assetsApi.wikiReviewCurationSuggestion(
					asset.id,
					suggestionId,
					decision,
				);
				...
				if (affectedPath && assetRoot) {
					dispatchFileTreeEditorCommand(
						"reload-file",
						"desktop-knowledge",
						`${assetRoot}/${affectedPath}`,
					);
				}
			} catch (reviewError) {
				toast.error(
					reviewError instanceof Error ? reviewError.message : "策展审阅失败",
				);
			} finally {
				setBusy(false);
			}
	...
	const uploadSelectedSources = useCallback(
		async (selectedFiles: SelectedKnowledgeSource[]) => {
			...
			const updateUpload = (
				id: string,
				stage: PendingKnowledgeUpload["stage"],
			) => {
			...
			if (succeeded > 0) {
				toast.success("资料已加入知识库", {
					description: `${succeeded} 个文件正在后台摄取`,
				});
			}
			await refreshMetadata();
		},
	...
	const handleChooseSources = useCallback(async () => {
		...
		try {
			...
			const files = await Promise.all(
				paths.map((path) =>
					invokeDesktop<SelectedKnowledgeSource>("read_knowledge_source", {
						path,
					}),
				),
			);
	...
	const handleReindex = useCallback(async () => {
		...
		try {
			...
			toast.error(
				reindexError instanceof Error ? reindexError.message : "刷新索引失败",
			);
		} finally {
			setBusy(false);
		}
	...
	const handleReingestSource = useCallback(
		async (sourcePath: string) => {
			...
			try {
				...
				toast.success("来源重新抽取已启动", {
					description: sourcePath.split("/").pop() || sourcePath,
				});
			} catch (reindexError) {
				toast.error(
	...
	const handleCancelIngest = useCallback(
		async (jobId: string) => {
			...
			try {
				...
				setIngestJobs((current) =>
					current.map((item) => (item.jobId === job.jobId ? job : item)),
				);
			} catch (cancelError) {
				toast.error(
					cancelError instanceof Error ? cancelError.message : "取消摄取失败",
				);
			} finally {
				setBusy(false);
			}
	...
	const handleRetryIngest = useCallback(
		async (jobId: string) => {
			...
			try {
				...
				toast.error(
					retryError instanceof Error ? retryError.message : "重试摄取失败",
				);
			} finally {
				setBusy(false);
			}
	...
	const handleSaveKnowledgeConfig = useCallback(
		async (embedding: WikiConfig["embedding"]) => {
			...
			setBusy(true);
			try {
				const config = await assetsApi.wikiUpdateConfig(asset.id, {
					embedding,
				});
	...
	const handleImportOkf = useCallback(
		async (files: FileList | null) => {
			...
			setBusy(true);
			try {
				const result = await assetsApi.wikiImportOkf(
					asset.id,
					await readFileAsBase64(file),
				);
				const warnings = result.validation.diagnostics.filter(
				...
				toast.error(
					importError instanceof Error ? importError.message : "OKF 导入失败",
				);
			} finally {
				setBusy(false);
	...
	const handleExportOkf = useCallback(async () => {
		...
		try {
			...
			const destination = await saveBase64File(
				result.filename,
				result.contentBase64,
				"application/zip",
			);
			if (destination === "cancelled") return;
			const errors = result.validation.diagnostics.filter(
			...
			toast.error(
				exportError instanceof Error ? exportError.message : "OKF 导出失败",
			);
		} finally {
			setBusy(false);
		}
	...
	return (
		<div className="flex h-full min-h-0 w-full flex-col bg-[#f7f7f5]">
				...
				<div className="flex min-w-0 flex-1 items-center gap-2">
					{canReturnToChat ? (
						<button
							...
							className="inline-flex size-8 shrink-0 items-center justify-center rounded-md border border-border bg-background text-foreground transition-colors hover:bg-muted"
							aria-label="返回对话原位置"
							title="返回对话"
						>
					...
					<div className="flex items-center gap-2">
						...
						>
							<RefreshCw
								className={cn(
									"size-3.5",
									busy || activeIngestJob ? "animate-spin" : "",
								)}
							/>
							{activeIngestProgress
								? `索引中 ${Math.round(activeIngestProgress.percent)}%`
								: "刷新索引"}
						</button>
					</div>
				)}
			...
			{fromChatSource ? (
				<output className="flex h-9 shrink-0 items-center border-b border-amber-200 bg-amber-50 px-3 text-xs text-amber-900">
					来源临时预览：当前页面只读，可选择、复制、查找和浏览其他来源；不能修改或保存。
				</output>
			) : null}
			...
			<div className="min-h-0 flex-1">
				{assetRoot ? (
					<AssetFileManager
						...
						persistSession={!fromChatSource}
						readOnly={fromChatSource}
						...
						headerSlot={
							<ExplorerHeader health={health} sources={sources.length} />
						}
						overviewSidebarPane={
							fromChatSource
								? undefined
								: {
										...
										content: (
											<OverviewPane
												...
												onReingestSource={(path) =>
													void handleReingestSource(path)
												}
												onOpenPath={(path) => {
													dispatchFileTreeEditorCommand(
								...
								? [
										{
											...
											content: (
												<CurationPane
													...
													onReview={(suggestionId, decision) =>
														void handleReviewCuration(suggestionId, decision)
													}
												/>
											),
										},
										...
										},
										{
											...
											content: (
												<KnowledgeSettingsPane
													...
													busy={busy}
													onSave={(embedding) =>
														void handleSaveKnowledgeConfig(embedding)
													}
												/>
											),
										},
