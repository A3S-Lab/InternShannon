...
export type SkillsPageSection = "config" | "personal" | "shared";

const SKILLS_PAGE_SECTIONS = new Set<SkillsPageSection>(["config", "personal", "shared"]);

export function getSkillsPageSectionFromSearch(search: string): SkillsPageSection {
  const params = new URLSearchParams(search);
  const section = params.get("section")?.trim();
  if (section === "defaults") return "config";
  return SKILLS_PAGE_SECTIONS.has(section as SkillsPageSection) ? (section as SkillsPageSection) : "config";
}
