...
import {
	connectSession,
	requestFreshSessionStatus,
	sendToSession,
} from "@/hooks/use-agent-ws";
import { resolveSessionRuntimeBusy } from "@/kernel/session/session-runtime-busy-state";
...
} from "./chat/agent-slash-command-state";
...
function buildAttachmentsFromImages(
	...
	| { uploadId: string; fileName: string; mimeType?: string; size?: number }[]
	| undefined {
...
async function writeNativeWorkspaceFile(
	path: string,
	content: string,
): Promise<void> {
...
function SelectionToolbar() {
	...
	const selectionVisible = state.visible;

	const positionToolbar = useCallback(
		(
			selectionRect: SelectionToolbarRect,
			toolbarSize = { width: 40, height: 38 },
		) => {
	...
	useLayoutEffect(() => {
		if (!selectionVisible || !selectionRect || !toolbarRef.current) return;
		const toolbarRect = toolbarRef.current.getBoundingClientRect();
		positionToolbar(selectionRect, {
			width: toolbarRect.width,
			height: toolbarRect.height,
		});
	}, [positionToolbar, selectionRect, selectionVisible]);
...
function isBypassMessage(msg: RichMessage): boolean {
	return (
		msg.role === "assistant" &&
		(msg.source as string | undefined) === "command:/btw"
	);
}
...
function getMessageText(msg: RichMessage): string {
	return msg.blocks
		.filter(
			(
				block,
			): block is Extract<(typeof msg.blocks)[number], { type: "text" }> =>
				block.type === "text",
		)
...
function formatTranscriptBlock(
	block: RichMessage["blocks"][number],
	index: number,
): string[] {
	const label = `${index + 1}.`;
	switch (block.type) {
...
function formatTranscriptMessage(msg: RichMessage, index: number): string {
	const header = [
		...
		...(msg.meta?.requestModel
			? [`REQUEST_MODEL: ${msg.meta.requestModel}`]
			: []),
		...(msg.meta?.responseModel
			? [`RESPONSE_MODEL: ${msg.meta.responseModel}`]
			: []),
...
function buildConversationTranscript({
	...
	const inProgressLines = [
		...(!hasStreamingSegments && streamingText?.trim()
			? [`TEXT:\n${streamingText.trim()}`]
			: []),
...
export default function AgentChat({
	...
	focusSourceAnchorId,
	focusMessageOffsetPx,
	focusMessageScrollerOffsetPx,
	onFocusMessageRestoreSettled,
	showHeader = true,
	onSend,
	onBeforeSend,
	onInterrupt,
	...
	/** Exact knowledge-source card identity within the focused message. */
	focusSourceAnchorId?: string;
	/** Viewport offset captured before navigating to a knowledge source. */
	focusMessageOffsetPx?: number;
	/** Message top relative to the chat scroll region, preferred over legacy viewport coordinates. */
	focusMessageScrollerOffsetPx?: number;
	onFocusMessageRestoreSettled?: (outcome: "restored" | "unavailable") => void;
	...
	onInterrupt?: () =>
		| AgentInputInterruptResult
		| Promise<AgentInputInterruptResult>;
	...
	const { revision: agentRegistryRevision } = useSnapshot(
		agentRegistryModel.state,
	);
	const rawMessages = messages[sessionId] || [];
	const safeMessages = useMemo(
		() => normalizeAgentChatMessages(rawMessages),
		[rawMessages],
	);
	...
	const currentSessionRuntime =
		resolveAgentChatSessionRuntimeState(currentSession);
	...
	const isRuntimeBusy = resolveSessionRuntimeBusy({
		runtimeStatus: sessionStatus[sessionId],
		runtimeBusy: currentSession?.runtimeBusy,
		isCompacting: currentSession?.isCompacting,
	});
	...
	const isExited =
		sdkSessions.find((s) => s.sessionId === sessionId)?.state === "exited";
	const state = useReactive({
		relaunching: false,
		...
		toolConfirmationDeliveryError: null as {
			requestId: string;
			message: string;
		} | null,
	...
	const displayMessages = useMemo(
		() =>
			richMessages.filter(
				(msg) =>
					!isBypassMessage(msg) &&
					!isBypassPromptMessage(msg) &&
					matchesSearch(msg, searchQuery),
			),
	...
	// A restored historical position must outlive the parent focus props. Route
	// state is acknowledged immediately after alignment, while queued streaming
	// and initial-scroll callbacks may still run afterwards. Keep those callbacks
	// from replacing the restored position until the user explicitly returns to
	// the bottom.
	const restoredPositionLockRef = useRef<string | null>(null);
	focusRestorePendingRef.current = isMessageFocusRestorePending({
		focusMessageId,
		focusMessageRequest,
	});
	if (focusRestorePendingRef.current && focusMessageId) {
		restoredPositionLockRef.current = `${sessionId}:${focusMessageId}:${focusMessageRequest}`;
	}
	...
	const scrollToBottom = useCallback((behavior: "smooth" | "auto" = "auto") => {
		if (
			...
			restoredPositionLockRef.current !== null
		)
			return;
	...
	useEffect(() => {
		...
		const hasPendingFocusedMessage =
			Boolean(focusMessageId) && focusMessageRequest > 0;
		if (hasPendingFocusedMessage) {
			if (displayMessages.some((message) => message.id === focusMessageId)) {
	...
	useEffect(() => {
		...
		// Route changes remount the chat surface. Do not turn that passive remount
		// into a runtime-config mutation while a reply or compaction is active.
		// Once the session becomes idle this effect runs again and safely syncs.
	...
	useEffect(() => {
		...
		const previousMessageCount = isNewSession
			? richMessages.length
	...
	}, [richMessages.length, sessionId, scrollToBottom]);
	...
	useEffect(() => {
		...
		prevStatusRef.current = currentSessionStatus;
	}, [currentSessionStatus, scrollToBottom]);
	...
	const handleDefaultSend = useCallback(
		async (
			...
			const ensureSessionConnected = async () => {
				if (
					agentModel.state.connectionStatus[targetSessionId] !== "connected"
				) {
			...
			const keepDraftInRelaunchedSession = () => {
				...
				agentModel.prefillChatInput(
					targetSessionId,
					text,
					images?.length ? { images } : false,
				);
			};
			const relaunchExitedSessionBeforeSend = async (): Promise<boolean> => {
				const processInfo = agentModel.state.sdkSessions.find(
				...
				try {
					targetSessionId = await relaunchSessionAndRebind({
						...
						preferAgentId:
							agentRegistryModel.resolveSessionAgentId(targetSessionId),
						apiUrl,
					});
					return true;
				} catch (error) {
			...
			const slashCommandAction = resolveAgentSlashCommandDispatchAction({
				...
				runtimeCommands:
					agentModel.state.sessions[targetSessionId]?.slashCommands,
				showStatusBar,
			...
			if (slashCommandAction.kind !== "none") {
				if (slashCommandAction.kind === "clear-session") {
					...
					const deliveryState = resolveClearSessionDeliveryState({
						sent: await sendSessionMessageWithRetry(targetSessionId, {
							type: "clear_session",
						}),
					});
				...
				if (slashCommandAction.kind === "local-info") {
					...
					if (commandName === "history") {
						...
						toast.info(
							`当前会话共 ${richMessages.length} 条消息，可直接输入关键词检索`,
						);
						return true;
					}
					if (commandName === "status") {
						const model =
							currentSession?.model || processInfo?.model || "未设置";
						const runtimeStatus = sessionStatus[targetSessionId] || "idle";
						const transportStatus =
							connectionStatus[targetSessionId] || "disconnected";
						toast.info(
							`模型 ${model} · 运行 ${runtimeStatus} · 连接 ${transportStatus}`,
						);
						return true;
					}
					if (commandName === "context") {
						await ensureSessionConnected();
						const refreshed = await requestFreshSessionStatus(targetSessionId);
						if (!refreshed) {
							toast.error("上下文状态读取失败，请重试");
							return true;
						}
						const contextSession = agentModel.state.sessions[targetSessionId];
						if (
							!Number.isFinite(contextSession?.contextUsedTokens) ||
							!Number.isFinite(contextSession?.contextMaxTokens)
						) {
							toast.error("上下文占用统计暂不可用，请重试");
							return true;
						}
						toast.info(
							formatAgentContextUsageMessage({
								usedTokens: contextSession?.contextUsedTokens,
								maxTokens: contextSession?.contextMaxTokens,
								percent: contextSession?.contextUsedPercent,
								estimated: contextSession?.contextUsageEstimated,
								pendingRefresh: contextSession?.contextUsagePendingRefresh,
							}),
						);
						return true;
					}
					...
					toast.info(
						names.length
							? `${commandName === "tools" ? "工具" : "技能"}：${names.join("、")}`
							: `当前会话未加载${commandName === "tools" ? "工具" : "技能"}`,
					);
					return true;
				}
			...
			const followDefaultModel =
				agentModel.state.sessions[targetSessionId]?.followDefaultModel;
			const routed = getSessionRoutingModel(sessionModel, followDefaultModel);
			const fullModel = formatResolvedRuntimeModel(routed);
			if (!fullModel) {
				...
				state.inputActionError = {
					message: `${SESSION_MODEL_CONFIGURATION_ERROR}消息草稿已保留。`,
					dismissLabel: "关闭发送错误提示",
				};
			...
			const permissionMode =
				agentModel.state.sessions[targetSessionId]?.permissionMode;
			...
			if (permissionMode || Object.keys(runtimeDefaults).length > 0) {
				const modelPatch = buildRoutedSessionModelPatch(
					fullModel,
					followDefaultModel,
				);
				try {
					...
					if (isStreamDebugEnabled()) {
						console.info(
							`[stream:${targetSessionId}] configureSession latency`,
							{
					...
					if (e instanceof AppError && e.code === 404) {
						...
						agentModel.setStreaming(targetSessionId, null);
						if (bypassTurnId) {
							agentModel.removeBypassConversation(
								targetSessionId,
								bypassTurnId,
							);
						}
					...
					if (e instanceof AppError && e.code === 409) {
						state.inputActionError = {
							message: "当前会话已有正在运行或压缩的任务，消息草稿已保留。",
							dismissLabel: "关闭发送错误提示",
						};
						keepDraftInRelaunchedSession();
						toast.info("当前会话正忙，请等待本轮完成后再发送。");
						sendToSession(targetSessionId, { type: "session_status" });
						return false;
					}
					const message =
					...
					state.inputActionError = {
						message: `会话模型配置失败，消息草稿已保留：${message}`,
						dismissLabel: "关闭发送错误提示",
					};
					agentModel.setBypassTurn(targetSessionId, false);
					agentModel.setSessionStatus(targetSessionId, "idle");
			...
			let sent = isBypassTurn
				...
				: sendToSession(targetSessionId, {
						...
						...(followDefaultModel === true ? {} : { model: fullModel }),
					});
			...
			if (!sent) {
				...
				console.warn(
					`[stream:${targetSessionId}] sendToSession failed — WS not connected`,
				);
				state.inputActionError = {
		...
		[
			...
			sessionStatus,
			showStatusBar,
			statusBarOptions?.showModelSwitcher,
		],
	);
		...
		async (
			...
			if (onBeforeSend) {
				try {
					...
					console.error(
						"onBeforeSend hook failed; sending original text",
						error,
					);
				}
			}

			const sendResult = await (onSend ?? handleDefaultSend)(
				outgoing,
				images,
				options,
			);
	...
	const handleRetry = useCallback(() => {
		...
		const retryUserMessage = safeMessages.find(
			(m) => m.id === retryState.userMessageId,
		);
		if (retryUserMessage) {
			void (onSend ?? handleDefaultSend)(
	...
	const handleDefaultInterrupt =
		useCallback(async (): Promise<AgentInputInterruptResult> => {
			state.inputActionError = null;
			...
			state.inputActionError = {
				message:
					"中断请求未送达。本地界面已停止等待，后台任务可能仍在运行，请在连接恢复后确认。",
				dismissLabel: "关闭中断错误提示",
			};
			toast.error("中断请求未送达，请检查本地服务连接");
			return false;
		}, [sessionId, settleInterruptedLocally]);
	...
	const handleInterrupt =
		...
		}, [handleDefaultInterrupt, onInterrupt]);
	...
	const handleToolConfirmation = useCallback(
		async (scope: "once" | "task" | "session") => {
			...
			if (!request) {
				console.warn(
					`[HITL] No confirmation request found for session ${sessionId}`,
				);
				return;
			}
			...
			console.log(
				`[HITL] User approved tool ${request.toolName} with scope ${scope}`,
			);
			...
			try {
				const sent = await sendToolConfirmationResponseWithRetry(
					sessionId,
					response,
				);
				...
				if (resolveToolConfirmationDeliveryAction({ sent }) === "keep") {
					...
					state.toolConfirmationDeliveryError = {
						requestId: request.requestId,
						message,
					};
					toast.error(message);
				...
				const message = formatToolConfirmationDeliveryError(
					error,
					"授权响应发送失败，请检查本地服务连接后重试",
				);
				state.toolConfirmationDeliveryError = {
					requestId: request.requestId,
					message,
		...
		[sessionId, toolConfirmationRequests],
	);
				...
				<div
					...
					className={cn(
						...
						highlightedMessageId === msg.id
							? "bg-primary/[0.06] shadow-[inset_0_0_0_1px_hsl(var(--primary)_/_0.22)]"
							: "",
					)}
				>
					...
					<MessageItem
						...
						sessionId={sessionId}
						focusSourceAnchorId={
							msg.id === focusMessageId && focusMessageRequest > 0
								? focusSourceAnchorId
								: undefined
						}
						onRetry={
							msg.id === retryState.assistantMessageId ? handleRetry : undefined
						}
	...
	const handleScrollerRef = useCallback(
		(scroller: HTMLElement | Window | null) => {
			...
			if (scrollerElRef.current === el && scrollerCleanupRef.current) return;
			scrollerCleanupRef.current?.();
			...
			state.showScrollBtn = !isNearBottom(el);
			scrollerCleanupRef.current = () => {
	...
	useEffect(() => {
		...
		const restoreLockId = `${sessionId}:${focusMessageId}:${focusMessageRequest}`;
		restoredPositionLockRef.current = restoreLockId;
		if (searchQuery.trim()) {
			state.searchQuery = "";
			return undefined;
		}
		const index = displayMessages.findIndex(
			(message) => message.id === focusMessageId,
		);
		if (index < 0) {
			if (displayMessages.length === 0) return undefined;
			const unavailableTimer = window.setTimeout(() => {
				if (restoredPositionLockRef.current === restoreLockId) {
					restoredPositionLockRef.current = null;
				}
				onFocusMessageRestoreSettled?.("unavailable");
			}, 1_500);
		...
		const settle = (outcome: "restored" | "unavailable") => {
			if (settled) return;
			settled = true;
			...
			onFocusMessageRestoreSettled?.(outcome);
		};
		const scheduleAlignment = (callback: () => void, delay = 0) => {
			if (delay > 0) {
				retryTimers.push(window.setTimeout(callback, delay));
				return;
			}
			animationFrames.push(window.requestAnimationFrame(callback));
		};
		const alignRenderedTarget = (attempt = 0) => {
			if (settled) return;
			const scroller = scrollerElRef.current;
			const messageTarget = scroller
				? Array.from(
						scroller.querySelectorAll<HTMLElement>("[data-agent-message-id]"),
					).find((element) => element.dataset.agentMessageId === focusMessageId)
				: null;
			const sourceTarget =
				scroller && focusSourceAnchorId
					? Array.from(
							scroller.querySelectorAll<HTMLElement>(
								"[data-knowledge-source-anchor]",
							),
						).find(
							(element) =>
								element.dataset.knowledgeSourceAnchor === focusSourceAnchorId,
						)
					: null;
			const waitingForExactSource =
				Boolean(focusSourceAnchorId) &&
				!sourceTarget &&
				performance.now() - startedAt < 1_500;
			if (!scroller || !messageTarget || waitingForExactSource) {
				if (performance.now() - startedAt < 2_500) {
					scheduleAlignment(() => alignRenderedTarget(attempt + 1), 60);
					return;
				}
				settle("unavailable");
				return;
			}
			// If a formerly expanded source group remounts collapsed, its exact card
			// can be absent. In that edge case restore the containing message instead
			// of ever falling through to the conversation bottom.
			const target = sourceTarget ?? messageTarget;
			const restoredExactSource = !focusSourceAnchorId || Boolean(sourceTarget);
			if (!resizeObserver && typeof ResizeObserver !== "undefined") {
				resizeObserver = new ResizeObserver(() => {
					stableAlignmentCount = 0;
					scheduleAlignment(() => alignRenderedTarget(attempt + 1));
				});
				resizeObserver.observe(scroller);
				resizeObserver.observe(target);
			}
			if (messageListRenderMode === "static" && attempt === 0) {
			...
			if (!hasMeasurableMessageFocusGeometry(geometry)) {
				stableAlignmentCount = 0;
				if (performance.now() - startedAt < 2_500) {
					scheduleAlignment(() => alignRenderedTarget(attempt + 1), 60);
					return;
			...
			const delta =
				restoredExactSource && Number.isFinite(focusMessageScrollerOffsetPx)
					? targetRect.top -
						scrollerRect.top -
						(focusMessageScrollerOffsetPx as number)
					: restoredExactSource && Number.isFinite(focusMessageOffsetPx)
						? targetRect.top - (focusMessageOffsetPx as number)
						: targetRect.top +
							targetRect.height / 2 -
			...
			if (Math.abs(delta) > 1) {
				...
				stableAlignmentCount = 0;
			} else if (isMessageFocusTargetVisible(geometry)) {
	...
	}, [searchQuery, searchState.currentIndex]);
	useEffect(() => {
	...
	useEffect(() => {
		const unsub = subscribe(agentModel.state, () => {
			...
			const ttsEnabled =
				readUserStorage("internshannon-tts-enabled") === "true";
		...
		<AgentSessionIdProvider sessionId={sessionId}>
			<div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden overflow-x-hidden bg-white font-sans text-foreground">
					...
					<ResizablePanel className="flex flex-col overflow-hidden">
						...
						>
							...
							!hasStreamingUi ? (
								...
								searchState.active &&
								!hasStreamingUi ? (
								<div className="flex h-full items-center justify-center text-sm text-foreground/80">
									没有匹配的消息
								</div>
									...
									<div className="relative h-full min-w-0 overflow-hidden">
										...
										{state.showScrollBtn && displayMessages.length > 0 ? (
											<button
												...
												className="absolute bottom-4 left-1/2 z-20 inline-flex size-9 -translate-x-1/2 items-center justify-center rounded-full border border-primary/30 bg-white text-primary shadow-[rgba(44,30,116,0.16)_0px_0px_15px] transition-colors hover:border-primary hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/40"
											>
									...
									<div className="relative h-full min-w-0 overflow-x-hidden">
										<Virtuoso
											...
											initialTopMostItemIndex={
												focusMessageId && focusMessageRequest > 0
													? Math.max(
															0,
				...
				<ToolConfirmationDialog
					...
					pending={
						state.pendingToolConfirmationRequestId ===
						toolConfirmationRequests[sessionId]?.requestId
					}
