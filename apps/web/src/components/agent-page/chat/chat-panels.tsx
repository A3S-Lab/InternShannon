			...
			<div className="flex items-center gap-2">
				...
				{status.output.length > 0 && (
					<button
						...
						aria-label={
							state.expanded ? "收起身份验证日志" : "展开身份验证日志"
						}
					>
						{state.expanded ? (
	...
	return (
		...
		>
			...
			<span className="font-medium">
				{disconnected ? "连接已断开,正在尝试重连…" : "正在连接书小安…"}
			</span>
		</div>
	);
}
...
async function waitForInboxSessionConnection(
	sessionId: string,
	timeoutMs = 1200,
): Promise<boolean> {
...
export function AgentMessageInbox({
	apiUrl,
	sessionId,
}: {
	apiUrl?: string;
	...
	const setMessageExecuting = useCallback(
		(messageId: string, executing: boolean) => {
			if (executing) {
				...
				state.executingMessageIds = Array.from(
					new Set([...state.executingMessageIds, messageId]),
				);
				return;
			}

			executingMessageIdsRef.current.delete(messageId);
			state.executingMessageIds = state.executingMessageIds.filter(
				(id) => id !== messageId,
			);
		},
		[],
	...
	const configureAndSend = useCallback(
		async (content: string): Promise<boolean> => {
			...
			const followDefaultModel =
				agentModel.state.sessions[sessionId]?.followDefaultModel;
			...
			const runtimeDefaults = getSessionRuntimeDefaults(
				agentRegistryModel.getSessionAgent(sessionId),
			);
			try {
				...
				toast.error("会话模型配置失败，Agent 消息未发送。");
				return false;
			}
	...
	const settleExecuteAttempt = useCallback(
		(
			...
			const message =
				errorMessage?.trim() || "Agent 消息未发送，请检查本地服务连接后重试。";
			if (action === "show_manual") {
				agentModel.updateAgentMessage(sessionId, messageId, {
					autoExecute: false,
						...
						>
							<div className="min-w-0 flex-1">
								<div className="mb-0.5 truncate text-[10px] text-muted-foreground">
									来自 {formatAgentMessageSourceLabel(msg.fromSessionId)} ·{" "}
									{msg.topic}
								</div>
...
export function EmptyChat({
	...
	const handleStarter = useCallback(
		(prompt: string) =>
			agentModel.prefillChatInput(sessionId, prompt, { autoSend: true }),
		[sessionId],
	);
	return (
