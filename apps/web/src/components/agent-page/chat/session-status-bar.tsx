...
import {
	EXECUTION_MODE_SELECT_LABEL,
	SESSION_MODEL_SELECT_LABEL,
} from "./session-status-bar-accessibility";
import {
...
function mainAgentDotClass(tone: MainAgentStatusTone) {
	if (tone === "disconnected")
		return "bg-red-500 shadow-[0_0_0_4px_rgba(239,68,68,0.10)]";
	if (tone === "connecting")
		return "bg-amber-400 shadow-[0_0_0_4px_rgba(245,158,11,0.12)]";
	if (tone === "running")
		return "bg-primary shadow-[0_0_0_4px_rgba(20,86,240,0.10)]";
	return "bg-emerald-500 shadow-[0_0_0_4px_rgba(16,185,129,0.10)]";
}

function stageClass(state: "active" | "done" | "idle") {
	if (state === "active")
		return "border-primary bg-primary text-white shadow-[0_0_0_4px_rgba(20,86,240,0.10)]";
	if (state === "done")
...
function runtimeRowClass(event: AgentRuntimeTimelineEvent) {
	if (event.status === "failed")
		return "border-red-100 bg-red-50/70 dark:bg-red-950/70";
	if (event.status === "waiting")
		return "border-amber-100 bg-amber-50/70 dark:bg-amber-950/70";
	if (event.status === "completed" || event.status === "ready")
...
export function SessionStatusBar({
	...
	const [actionError, setActionError] =
		useState<SessionStatusBarActionErrorState | null>(null);
	const [modelSwitcherHighlighted, setModelSwitcherHighlighted] =
		useState(false);
	const modelSwitcherTriggerRef = useRef<HTMLSelectElement>(null);
	const previousModelSwitcherFocusRequestRef = useRef(
		modelSwitcherFocusRequest,
	);
	const actionErrorPresentation = useMemo(
		() => resolveSessionStatusBarActionError(actionError),
		[actionError],
	);
	...
	const latestTool =
		...
		activeToolProgress[sessionId] ??
		null;
	const recentCompletedTools = (completedTools[sessionId] ?? [])
		.slice(-3)
		.reverse();
	...
	const latestMainRuntimeEvent = [...runtimeEvents]
		.reverse()
		.find((event) => event.kind === "main_agent");
	const latestToolRuntimeEvent = [...runtimeEvents]
		.reverse()
		.find((event) => event.kind === "tool");
	const latestToolRuntimeEventsByKey = new Map<
		string,
		AgentRuntimeTimelineEvent
	>();
	for (const event of runtimeEvents) {
		if (event.kind !== "tool") continue;
		latestToolRuntimeEventsByKey.set(
			event.toolUseId || event.toolName || event.id,
			event,
		);
	}
	const activeRuntimeToolEvents = Array.from(
		latestToolRuntimeEventsByKey.values(),
	)
		.filter((event) => isLiveRuntimeStatus(event.status))
		.slice(-3);
	const recentRuntimeEvents = runtimeEvents.slice(-8).reverse();
	const runtimeBusy = resolveSessionRuntimeBusy({
		runtimeStatus,
		runtimeBusy: session?.runtimeBusy,
		isCompacting: session?.isCompacting,
		activeToolCount: activeTools.length,
		hasLiveRuntimeEvent: isLiveRuntimeStatus(latestMainRuntimeEvent?.status),
	});
			...
			state:
				activeTools.length > 0 || activeRuntimeToolEvents.length > 0
					...
					: recentCompletedTools.length > 0 ||
							latestToolRuntimeEvent?.status === "completed"
						? "done"
						: "idle",
		},
		{
			label: "输出",
			state:
				latestMainRuntimeEvent?.phase === "model_output" ||
				latestMainRuntimeEvent?.phase === "finalize"
					? "active"
	...
	const availableModels = useMemo(() => {
		return buildStatusBarModelOptions(settingsSnap.providers);
	...
	const currentModelValue = useMemo(() => {
		...
		return resolveStatusBarModelValue({
			...
			routedModel: getSessionRoutingModel(
				rawModel,
	...
	const modelOptions = useMemo(() => {
		return includeCurrentStatusBarModelOption(
			availableModels,
			currentModelValue,
		);
	}, [availableModels, currentModelValue]);
	const hasFocusableModelSwitcher = Boolean(
		session && availableModels.length > 0 && !switchingModel && !runtimeBusy,
	);
	...
	const sessionPermissionMode = resolveSessionPermissionMode(
		session?.permissionMode,
	);
	const sessionModelDisplayText = resolveSessionModelDisplayText(
		session?.model,
	);
	const currentExecutionMode = EXECUTION_MODE_OPTIONS.find(
		(option) => option.value === sessionPermissionMode,
	) ?? {
		value: sessionPermissionMode,
		label: `兼容模式 (${sessionPermissionMode})`,
		description: "该模式来自历史会话；选择其他模式后才会更新会话设置。",
	};
	const executionModeOptions = EXECUTION_MODE_OPTIONS.some(
		(option) => option.value === sessionPermissionMode,
	)
		? EXECUTION_MODE_OPTIONS
		: [currentExecutionMode, ...EXECUTION_MODE_OPTIONS];

	const showActionError = (
		kind: SessionStatusBarActionErrorState["kind"],
		message: string,
	) => {
	...
	const handleModelChange = async (value: string) => {
		if (!session) return;
		if (runtimeBusy) {
			toast.info("当前会话正在运行或压缩，请完成后再切换模型。");
			return;
		}
		...
		try {
			...
			const message = formatSessionStatusBarActionError(
				error,
				"模型没有切换成功，请检查本地服务连接后重试。",
			);
	...
	const handleExecutionModeChange = (mode: string) => {
		if (!session) return;
		if (runtimeBusy) {
			toast.info("当前会话正在运行或压缩，请完成后再切换执行模式。");
			return;
		}
		const planningPatch =
			mode === "plan"
		...
		if (!sent) {
			...
			showActionError(
				"execution-mode",
				"执行模式没有送达本地运行时，请恢复连接后重试。",
			);
			return;
		}
		agentModel.updateSession(sessionId, { permissionMode: mode });
		void agentApi
			.updateSession(sessionId, sessionPatch, apiUrl)
			.catch((error) => {
				if (error instanceof AppError && error.code === 404) {
					void handleMissingSession({ sessionId, apiUrl });
					return;
				}
				if (error instanceof AppError && error.code === 409) {
					agentModel.updateSession(sessionId, { permissionMode: previousMode });
					showActionError(
						"execution-mode",
						"当前会话正在运行或压缩，执行模式未切换。",
		...
		<div className="flex w-full max-w-full select-none flex-col gap-1 overflow-hidden border-t border-border-light bg-white px-2 py-1 text-[10px] leading-none text-foreground/80 shadow-[rgba(0,0,0,0.04)_0px_2px_4px] sm:flex-row sm:items-center sm:gap-1.5">
				...
				{!readonlyCwd && (
					<div className="flex h-6 shrink-0 items-center gap-1 rounded-full border border-border-light bg-muted/40 px-2">
						...
						<select
							...
							value={sessionPermissionMode}
							onChange={(event) =>
								handleExecutionModeChange(event.currentTarget.value)
							}
							disabled={!session || runtimeBusy}
							className="h-5 max-w-[112px] cursor-pointer appearance-auto border-0 bg-transparent p-0 text-[10px] text-foreground/80 outline-none disabled:cursor-not-allowed disabled:opacity-50"
						>
				...
				{showModelSwitcher && (
					<div
						className={cn(
							...
							modelSwitcherHighlighted &&
								"border-primary/60 bg-primary/5 shadow-[0_0_0_3px_rgba(20,86,240,0.10)]",
						)}
					>
						...
						{modelOptions.length > 0 ? (
							<select
								...
								className="h-5 max-w-[136px] cursor-pointer appearance-auto truncate border-0 bg-transparent p-0 text-[10px] text-foreground/80 outline-none disabled:cursor-not-allowed disabled:opacity-50 sm:max-w-[160px]"
							>
			...
			{actionErrorPresentation ? (
				...
				>
					...
					<span className="shrink-0 text-[10px] font-medium leading-5">
						{actionErrorPresentation.title}
					</span>
					<span className="min-w-0 truncate text-[10px] leading-5">
						{actionErrorPresentation.message}
					</span>
					<button
						type="button"
				...
				<Popover>
					<PopoverTrigger asChild>
						...
						>
							...
							<span
								className={`size-1.5 shrink-0 rounded-full ${mainRuntimeDotClass}`}
							/>
									...
									{mainStages.map((stage, index) => (
										<div key={stage.label} className="flex flex-1 items-center">
											...
											{index < mainStages.length - 1 ? (
												<span
													className={`mx-1 h-px flex-1 ${stageLineClass(stage.state)}`}
												/>
											) : null}
										</div>
									))}
									...
									{latestMainRuntimeEvent ? (
										<div className="rounded-[10px] border border-primary/30 bg-primary/5 px-2 py-1.5">
											<div className="flex items-center gap-1.5 text-[11px] font-medium text-foreground">
												<span
													className={`size-1.5 rounded-full ${runtimeDotClass(latestMainRuntimeEvent)}`}
												/>
										...
										<div className="rounded-[10px] border border-border-light bg-muted/40 px-2 py-1.5">
											...
											{latestTool.phase === "input_streaming" ? (
												<p className="mt-1 text-[10px] text-muted-foreground">
													模型正在生成工具参数
													{latestTool.inputDeltaCount
														? ` · ${latestTool.inputDeltaCount} 段`
														: ""}
												</p>
											) : null}
											{latestTool.input ? (
												<p className="mt-1 truncate text-[10px] text-foreground/80">
													{latestTool.input}
												</p>
											) : null}
										</div>
									...
									{activeRuntimeToolEvents.length > 0 ? (
										<div className="space-y-1 rounded-[10px] border border-border-light bg-muted/20 px-2 py-1.5">
											...
											{activeRuntimeToolEvents.map((event) => (
												...
												>
													<span
														className={`size-1.5 shrink-0 rounded-full ${runtimeDotClass(event)}`}
													/>
													<span className="truncate">
									...
									{recentCompletedTools.length > 0 ? (
										<div className="space-y-1">
											...
											{recentCompletedTools.map((tool) => (
												...
												>
													<CheckCircle2
														className={`size-3 ${tool.is_error ? "text-red-500" : "text-emerald-500"}`}
													/>
									...
									{recentRuntimeEvents.length > 0 ? (
										<div className="space-y-1 rounded-[12px] bg-muted/40 p-2">
											...
											{recentRuntimeEvents.map((event) => (
												<div
													key={event.id}
													className={`rounded-[10px] border px-2 py-1.5 ${runtimeRowClass(event)}`}
												>
													...
													{event.detail ? (
														<p className="mt-1 line-clamp-2 text-[10px] leading-4 text-foreground/80">
															{event.detail}
														</p>
													) : null}
