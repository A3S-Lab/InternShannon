...
test("keeps the toolbar mounted while its copy button receives the click", () => {
	assert.match(
		agentChatSource,
		/event\.target instanceof Node\s*&&\s*toolbarRef\.current\?\.contains\(event\.target\)\s*\)\s*return;/,
	...
	assert.match(
		agentChatSource,
		/const handleMouseUp = \(event: MouseEvent\) => \{\s*if \(\s*event\.target instanceof Node\s*&&\s*toolbarRef\.current\?\.contains\(event\.target\)\s*\)\s*return;/,
