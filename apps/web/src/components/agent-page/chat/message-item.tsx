...
import { splitUniqueFileMentions } from "./message-source-state";
...
function FileTypeIcon({ ext, size = 22 }: { ext?: string; size?: number }) {
	...
	// Config / data
	if (
		["json", "toml", "yaml", "yml", "xml", "ini", "env"].includes(ext || "")
	) {
		return (
			<svg
				aria-hidden="true"
...
function KnowledgeCitationCard({ citation }: { citation: string }) {
	...
	return (
		<button
			...
			onClick={() =>
				navigate(
					`/knowledge?source=${encodeURIComponent(citation)}&open=${Date.now()}`,
				)
			}
...
function TextWithFileMentions({
	...
	const segments = useMemo(
		() => splitUniqueFileMentions(safeContent),
		[safeContent],
	);
	...
	if (!hasFileMentions) {
		return (
			<MemoizedMarkdown id={safeContent.slice(0, 32)} content={safeContent} />
		);
	}
		...
		isCompactLeft
			? cn(
					...
					isUser
						...
						: "border-border-light bg-white text-foreground",
				)
			: cn(
					"px-3 py-2",
					isUser
			...
			>
				...
				>
					<div
						className={cn(
							"relative h-5",
							isCompactLeft ? "mb-0.5 pl-0.5" : "mb-0.5",
						)}
					>
						<time
							className={cn(
					...
					<div className="relative">
						<span
							aria-hidden="true"
							className={cn(
								"pointer-events-none absolute top-3.5 hidden size-3 rotate-45",
								"z-0",
							)}
						/>
						<span
							aria-hidden="true"
							className={cn(
								"pointer-events-none absolute top-[15px] hidden size-2 rotate-45",
								"z-0",
							)}
						/>
