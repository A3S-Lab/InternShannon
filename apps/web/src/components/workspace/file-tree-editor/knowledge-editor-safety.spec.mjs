...
const knowledgeExplorerPaneSource = readFileSync(
  fileURLToPath(new URL("../../../desktop/pages/knowledge/components/knowledge-explorer-pane.tsx", import.meta.url)),
  "utf8",
);
const knowledgeGraphPaneSource = readFileSync(
  fileURLToPath(new URL("../../../desktop/pages/knowledge/components/knowledge-graph-pane.tsx", import.meta.url)),
  "utf8",
);
const knowledgeCurationPaneSource = readFileSync(
  fileURLToPath(new URL("../../../desktop/pages/knowledge/components/knowledge-curation-pane.tsx", import.meta.url)),
  "utf8",
);
const knowledgeSettingsPaneSource = readFileSync(
  fileURLToPath(new URL("../../../desktop/pages/knowledge/components/knowledge-settings-pane.tsx", import.meta.url)),
  "utf8",
);
const knowledgePageUtilsSource = readFileSync(
  fileURLToPath(new URL("../../../desktop/pages/knowledge/knowledge-page-utils.ts", import.meta.url)),
  "utf8",
);
...
test("KnowledgePage opens overview results without replacing the overview sidebar", () => {
  ...
  assert.match(knowledgeExplorerPaneSource, /onClick=\{\(\) => props\.onOpenPath\(page\.path\)\}/);
});

test("KnowledgePage can restore and persist the local retrieval defaults", () => {
  assert.match(knowledgePageUtilsSource, /DEFAULT_KNOWLEDGE_EMBEDDING[\s\S]*?model: "local-hash-v1"/);
  assert.match(knowledgePageUtilsSource, /keywordWeight: 1[\s\S]*?vectorWeight: 6[\s\S]*?mmrLambda: 0\.78/);
...
test("KnowledgePage renders real graph edges without a synthetic Vault center", () => {
  ...
  assert.match(knowledgeGraphPaneSource, /markerEnd="url\(#knowledge-graph-arrow\)"/);
  assert.match(knowledgeGraphPaneSource, /setSelectedEdgeKey\(edgeKey\(edge\)\)/);
...
test("KnowledgePage keeps the original knowledge-base name and summary", () => {
  ...
  assert.match(knowledgeExplorerPaneSource, /formatRelativeTime\(props\.health\.lastIngestedAt\)/);
});
...
test("Office editors keep one runtime per file and avoid competing DOM cleanup", () => {
  ...
  assert.match(sources[2], /runtime\.canvasView\.activePage\(entry\.pageId, runtime\.slide\.getUnitId\(\)\)/);
  ...
  assert.match(sources[0], /baselineSnapshot: runtimeRef\.current\.baselineSnapshot/);
  assert.match(sources[0], /baselineSnapshot: structuredClone\(document\.getSnapshot\(\)\)/);
  ...
  assert.match(sources[1], /baselineSnapshot: runtimeRef\.current\.baselineSnapshot/);
  assert.match(sources[1], /baselineSnapshot: structuredClone\(workbook\.save\(\)\)/);
