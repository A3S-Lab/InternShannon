...
} from "./markdown-mode-selection";
import { buildMarkdownSourceDecorations } from "./markdown-source-decorations";
import {
	FileHistoryDialog,
...
import {
	dispatchFileEditorSave,
	FILE_EDITOR_SAVE_EVENT,
	FILE_EDITOR_SAVE_ALL_EVENT,
	FILE_TREE_EDITOR_COMMAND_EVENT,
	type FileEditorSaveDetail,
...
} from "@/components/agent-page/chat/selection-toolbar-state";
import {
	createEditorLoadIdentity,
	hasEditorBufferChanged,
	hasPendingEditorSave,
} from "./editor-save-contract";
import "./styles.css";
...
function getFileTabIconElement(name: string, ext: string): React.ReactNode {
	...
	if (["js", "jsx"].includes(ext))
		return <FileCode2 className="size-4 text-[#f0db4f]" />;
	if (["ts", "tsx"].includes(ext))
		return <FileCode2 className="size-4 text-[#3178c6]" />;

	// Backend languages
	if (["py"].includes(ext)) return <Code className="size-4 text-[#3776ab]" />;
	if (["kt", "kts"].includes(ext))
		return <Code className="size-4 text-[#7f52ff]" />;
	...
	if (["java", "c", "cpp", "h", "hpp"].includes(ext))
		return <Code className="size-4 text-[#dc143c]" />;

	// Web
	if (["html", "htm"].includes(ext))
		return <Globe className="size-4 text-[#e34c26]" />;
	if (["css", "scss", "sass", "less"].includes(ext))
		return <FileCode className="size-4 text-[#264de4]" />;
	if (["vue"].includes(ext)) return <Code className="size-4 text-[#4fc08d]" />;
	if (["svelte"].includes(ext))
		return <Code className="size-4 text-[#ff3e00]" />;

	// Config / Data
	if (["json", "jsonc"].includes(ext))
		return <Braces className="size-4 text-[#f59e0b]" />;
	if (["yaml", "yml"].includes(ext))
		return <FileText className="size-4 text-[#cb171e]" />;
	if (["toml"].includes(ext))
		return <FileText className="size-4 text-[#9422ff]" />;
	if (["ini", "conf", "env"].includes(ext))
	...
	if (["txt", "rst"].includes(ext))
		return <FileText className="size-4 text-muted-foreground" />;
	if (["pdf"].includes(ext))
		return <FileText className="size-4 text-[#dc2626]" />;
	...
	if (["mp4", "mov", "avi", "mkv", "webm"].includes(ext))
		return <Video className="size-4 text-[#ec4899]" />;
	if (["mp3", "wav", "ogg", "flac", "aac"].includes(ext))
		return <Music className="size-4 text-[#22c55e]" />;

	// Archives
	if (["zip", "tar", "gz", "rar", "7z", "bz2"].includes(ext))
		return <Archive className="size-4 text-[#d97706]" />;

	// Database
	if (["db", "sqlite", "sql"].includes(ext))
		return <Database className="size-4 text-[#0891b2]" />;

	// Shell / Scripts
	if (["sh", "bash", "zsh", "fish"].includes(ext))
		return <Terminal className="size-4 text-[#22c55e]" />;
	...
	return (
		...
		>
			...
			{isPreview && showPreviewIndicator && (
				<span
					className="dv-default-tab-preview"
					title="预览，双击保持打开"
					aria-hidden="true"
				>
			...
			{isVsCodeTab && isDirty && (
				<span className="dv-default-tab-dirty" title="未保存" aria-hidden="true" />
			)}
			{!hideClose && (
				<button
					type="button"
...
let _pdOnDrop:
	| ((src: { path: string; isDir: boolean } | null, dest: string | null) => void)
	| null = null;
...
export interface FileTreeEditorProps {
	...
	/** Disable interval/blur auto-save. Manual save commands remain available. */
	...
	openFileRequest?: {
		path: string;
		nonce: string | number;
		/** Open as a disposable preview instead of accumulating a pinned editor tab. */
		preview?: boolean;
		pinned?: boolean;
	} | null;
...
type EditorSaveResult = "saved" | "clean" | "read-only" | "stale" | "error";
...
function normalizeMenuItems(
	items: MenuItem[],
	capabilities: MenuRuntimeCapabilities,
): MenuItem[] {
...
function filterTreeByQuery(root: FsNode | null, query: string): FsNode | null {
	...
	const visit = (node: FsNode): FsNode | null => {
		...
		const children = (node.children ?? [])
			.map(visit)
			.filter((child): child is FsNode => !!child);
...
function EditorBreadcrumbs({
	...
	for (const segment of segments) {
		breadcrumbKey = `${breadcrumbKey}/${segment}`;
		breadcrumbItems.push({
			key: breadcrumbKey,
			segment,
			isFirst: breadcrumbItems.length === 0,
			isCurrent: breadcrumbItems.length === segments.length - 1,
		});
	}
	...
	return (
		<div className="file-tree-editor-breadcrumbs">
			<div className="file-tree-editor-breadcrumb-path">
				{breadcrumbItems.map((item) => (
					<React.Fragment key={item.key}>
						{!item.isFirst && (
							<ChevronRight
								className="file-tree-editor-breadcrumb-separator"
								aria-hidden="true"
							/>
						)}
						<span
							className={cn(
								"file-tree-editor-breadcrumb-item",
			...
			{actions && (
				<div className="file-tree-editor-breadcrumb-actions">{actions}</div>
			)}
		</div>
	);
}
...
function getFileTypeDisplayName(
	fileType: FileType | null,
	path: string | null,
) {
	if (!path) return "未打开文件";
	if (fileType === "text")
		return getLanguageDisplayName(getMonacoLanguage(path));
...
function FileTreeIdeStatusBar({
	...
	const relativePath = activeFile
		? getWorkspaceRelativePath(rootPath, activeFile)
		: "";
	...
	return (
		<footer className="file-tree-ide-statusbar">
			<button
				type="button"
			...
			{dirtyFileCount > 0 ? (
				<>
					<EditorStatusDivider />
					<span className="file-tree-editor-statusitem is-dirty">
						{dirtyFileCount} 未保存
					</span>
				</>
			) : null}
			{activeStatus ? (
				<>
					<EditorStatusDivider />
					<EditorSaveStatus
						status={activeStatus.saveStatus}
						lastSavedAt={activeStatus.lastSavedAt}
					/>
				</>
			) : null}
			{readOnly ? (
			...
			<span className="file-tree-editor-statusitem file-tree-ide-statusfile">
				{relativePath || "就绪"}
			</span>
				...
				{activeStatus ? (
					<>
						<span className="file-tree-editor-statusitem is-secondary">
							{activeStatus.lineCount} 行
						</span>
						<EditorStatusDivider />
						<span className="file-tree-editor-statusitem is-secondary">
							{activeStatus.characterCount} 字符
						</span>
						<EditorStatusDivider />
						{activeStatus.selectionLength > 0 ? (
							<>
								<span className="file-tree-editor-statusitem">
									{activeStatus.selectionLength} 已选
								</span>
								<EditorStatusDivider />
							</>
						) : null}
						<span className="file-tree-editor-statusitem">
							行 {activeStatus.cursorPosition.line}, 列{" "}
							{activeStatus.cursorPosition.column}
						</span>
						<EditorStatusDivider />
						<span className="file-tree-editor-statusitem is-secondary">
...
const FILE_TREE_LAYOUT_STORAGE_PREFIX =
	"internshannon:file-tree-editor:layout:v3";
const FILE_TREE_EDITOR_SESSION_STORAGE_PREFIX =
	"internshannon:file-tree-editor:session";
let activeFileTreeRoot: HTMLElement | null = null;

function shallowEqualObjectParams(
...
function buildPaneviewLayoutStorageKey(
...
async function updateLocalSnapshotLabel(
	filePath: string,
	snapshotId: string,
	label: string,
) {
	...
	const next = snapshots.map((snapshot) =>
		snapshot.id === snapshotId ? { ...snapshot, label } : snapshot,
	);
...
function readCachedPaneviewLayout(
	...
	try {
		return readUserJsonStorage<CachedPaneviewLayout | null>(
			buildPaneviewLayoutStorageKey(rootPath, area),
			null,
		);
	} catch {
		return null;
...
function writeCachedPaneviewLayout(
	...
	try {
		writeUserJsonStorage(
			buildPaneviewLayoutStorageKey(rootPath, area),
			stripPaneviewLayoutParams(layout),
		);
	} catch (e) {
...
function PaneHeader({
	...
	params,
	api,
}: IPaneviewPanelProps<ExplorerPaneParams>) {
	const isExplorer = api.id === "explorer";
	const explorerParams =
		isExplorer && params ? (params as ExplorerPaneParams) : null;
	...
	return (
		<div className="file-tree-pane-header flex h-full items-center gap-1.5 px-2">
			...
			>
				...
				<span className="file-tree-pane-title min-w-0 flex-1 truncate">
					{title}
				</span>
			</button>
...
function SimplePaneHeader({
	title,
	api,
}: IPaneviewPanelProps<Record<string, unknown>>) {
	const toggleExpanded = () => api.setExpanded(!api.isExpanded);
...
function PanelLoadingState({ label }: { label: string }) {
	return (
		<div className="file-tree-panel-state" aria-live="polite">
			<Loader2
				className="file-tree-panel-state-icon animate-spin"
				aria-hidden="true"
			/>
...
function PanelErrorState({
	...
	return (
		<div
			className="file-tree-panel-state file-tree-panel-state-error"
			role="alert"
		>
...
function PanelEmptyState({
	...
	return (
		<div className="file-tree-panel-state">
			...
			{description && (
				<p className="file-tree-panel-state-description">{description}</p>
			)}
...
function VscodeExplorerSectionHeader({
	...
	const content = (
		<>
			<ChevronDown
				className={cn("file-tree-vscode-section-chevron", !open && "is-collapsed")}
				aria-hidden="true"
			/>
...
function ExplorerPane({ params }: IPaneviewPanelProps<ExplorerPaneParams>) {
	...
	const searchValue =
		typeof rawPane?.searchQuery === "string" ? rawPane.searchQuery : "";
	const selectedPaths = Array.isArray(rawPane?.selectedPaths)
		? rawPane.selectedPaths
		: [];
	...
	const pane: ExplorerPaneParams = {
		...
		treeLoadError:
			typeof rawPane?.treeLoadError === "string" ? rawPane.treeLoadError : null,
		partialLoadErrorCount:
			typeof rawPane?.partialLoadErrorCount === "number"
				? rawPane.partialLoadErrorCount
				: 0,
		tree,
		nodeByPath:
			rawPane?.nodeByPath instanceof Map
				? rawPane.nodeByPath
				: buildTreeNodeIndex(tree),
		activeFile:
			typeof rawPane?.activeFile === "string" ? rawPane.activeFile : null,
		selectedPaths,
		selectedPathSet:
			rawPane?.selectedPathSet instanceof Set
				? rawPane.selectedPathSet
				: new Set(selectedPaths),
		searchQuery: searchValue,
		totalFiles: typeof rawPane?.totalFiles === "number" ? rawPane.totalFiles : 0,
		totalFolders:
			typeof rawPane?.totalFolders === "number" ? rawPane.totalFolders : 0,
		openNodes: rawPane?.openNodes instanceof Set ? rawPane.openNodes : new Set(),
		autoExpandDepth:
			typeof rawPane?.autoExpandDepth === "number" ? rawPane.autoExpandDepth : 1,
		collapseAllVersion:
			typeof rawPane?.collapseAllVersion === "number"
				? rawPane.collapseAllVersion
				: 0,
		showSearchFilter: rawPane?.showSearchFilter !== false,
		showFooter: rawPane?.showFooter !== false,
		dirtyPaths:
			rawPane?.dirtyPaths instanceof Set ? rawPane.dirtyPaths : new Set(),
		dragSrc: rawPane?.dragSrc ?? null,
		dropTarget:
			typeof rawPane?.dropTarget === "string" ? rawPane.dropTarget : null,
		dropIndicator: rawPane?.dropIndicator ?? null,
		renamingPath:
			typeof rawPane?.renamingPath === "string" ? rawPane.renamingPath : null,
		cutPaths: rawPane?.cutPaths instanceof Set ? rawPane.cutPaths : new Set(),
		treeFocusRef: rawPane?.treeFocusRef ?? fallbackTreeFocusRef,
		emptyAreaItems: Array.isArray(rawPane?.emptyAreaItems)
			? rawPane.emptyAreaItems
			: [],
	...
	const rootLabel = pane.rootPath
		? getBaseName(pane.rootPath) || "工作区"
		: "工作区";
	const rootMeta =
		pane.rootPath && !pane.loading
			? `${pane.totalFiles} 文件 / ${pane.totalFolders} 文件夹`
			: undefined;
		...
		pane.onInlineCreateCancel ? (
			<div className="file-tree-inline-create-row" style={{ paddingLeft: "14px" }}>
				{pane.inlineCreate.type === "folder" ? (
					<Folder
						className="file-tree-inline-create-icon text-[#eab308]"
						aria-hidden="true"
					/>
				) : (
					<File
						className="file-tree-inline-create-icon text-muted-foreground"
						aria-hidden="true"
					/>
				)}
				<InlineCreationInput
	...
	return (
		<div
			className={cn(
				"flex h-full min-h-0 flex-col bg-muted/[0.03]",
				isVsCodePane && "file-tree-explorer-pane-vscode",
			)}
		>
			...
			{pane.rootPath && pane.showSearchFilter && (
				<div className="file-tree-explorer-search">
					{isSearchPending ? (
						<Loader2
							className="file-tree-explorer-search-icon animate-spin"
							aria-hidden="true"
						/>
					) : (
			...
			{pane.afterSearchSlot && (
				<div className="px-2 py-1.5">{pane.afterSearchSlot}</div>
			)}
			...
			{showWorkspaceTree && (
					...
					onKeyDown={(e) => {
						...
						if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
							...
							if (e.key === "ArrowLeft") {
								...
								if (
									parentPath &&
									parentPath !== pane.rootPath &&
									pane.nodeByPath.has(parentPath)
								) {
				...
				>
					{!pane.rootPath ? (
						...
						<PanelErrorState
							message={pane.treeLoadError}
							onRetry={pane.onRefreshTree}
						/>
			...
			{pane.showFooter && (
				...
				>
					...
					{pane.selectedPaths.length > 0 && (
						<span>{pane.selectedPaths.length} 已选</span>
					)}
					{(pane.dirtyPaths?.size ?? 0) > 0 && (
						<span className="is-dirty">{pane.dirtyPaths?.size} 未保存</span>
					)}
					{pane.partialLoadErrorCount > 0 && (
						<span className="is-warning">
							{pane.partialLoadErrorCount} 个目录未加载
						</span>
					)}
				</output>
			)}
...
function formatShortcut(...keys: string[]) {
	...
	if (!isApple)
		return keys.map((key) => (key === "Cmd" ? "Ctrl" : key)).join("+");
...
function isFileTreeDebugEnabled(): boolean {
	try {
		return (
			globalThis.localStorage?.getItem("internshannon-file-tree-debug") === "true"
		);
	} catch {
		return false;
	}
...
function buildStaticFilesTree(
	rootPath: string,
	files: readonly FileTreeEditorStaticFile[],
): WorkspaceTreeBuildResult {
	const root: FsNode = {
	...
	for (const file of files) {
		...
		parent.children = [
			...(parent.children ?? []).filter(
				(child) => !(child.path === joinWorkspacePath(currentPath, fileName)),
			),
			{
				name: fileName,
...
function buildStaticFileContentMap(
	rootPath: string,
	files: readonly FileTreeEditorStaticFile[],
) {
		...
		async (entry) => {
			...
			try {
				...
				for (const sample of child.partialLoadErrorSamples) {
					appendErrorSample(
						diagnostics.partialLoadErrorSamples,
						sample,
...
async function fetchTreeWithDiagnostics(
	path: string,
	depth = 6,
): Promise<WorkspaceTreeBuildResult> {
			...
			{menu.items.map((item, i) =>
				item.separator ? (
					...
					>
						...
						{item.shortcut && (
							<span className="file-tree-context-menu-shortcut">{item.shortcut}</span>
						)}
					</button>
				),
			)}
	...
	const onMove = (me: PointerEvent) => {
		...
		if (_pdActive && _pdGhost) {
			...
			if (itemEl && itemPath && itemPath !== path) {
				...
				candidate &&
				candidate !== path &&
				!(isDir && candidate.startsWith(`${path}/`))
			) {
...
const TreeNode = React.memo(function TreeNode({
	...
	const directoryOpen = state.open;
	useEffect(() => {
	...
	useEffect(() => {
		...
		frame = window.requestAnimationFrame(() => {
			document
				.querySelector<HTMLElement>(getDataPathSelector(node.path))
				?.scrollIntoView({
					block: "nearest",
					inline: "nearest",
				});
		...
		return (
			<div className="file-tree-node" data-dir-path={node.path}>
				<button
					...
					onClick={(e) => {
						const clickedDisclosure = (e.target as HTMLElement | null)?.closest(
				...
				>
					<span className="file-tree-row-content">
						<span
							className="file-tree-row-disclosure"
						...
						{state.childError && (
							<XCircle
								className="ml-auto size-3 shrink-0 text-destructive/70"
								aria-label="目录加载失败"
							/>
						)}
						{dropIndicator?.path === node.path &&
				...
				{(forceOpen || state.open) && state.children !== null ? (
					<fieldset
						className="file-tree-node-children m-0 min-w-0 border-0 p-0"
					...
					>
						...
						{state.childError && (
							<div
								className="file-tree-inline-error"
								style={{ marginLeft: `${(depth + 1) * 14 + 18}px` }}
							>
								{state.childError}
							</div>
						)}
						...
						{inlineCreate?.parentPath === node.path && (
							<div
								className="file-tree-inline-create-row"
	...
	const isOffice = OFFICE_EXTS.has(
	...
	return (
		...
		>
			<span className="file-tree-row-content is-file">
				...
				{isDirtyFile && (
					<span className="file-tree-dirty-dot" title="已修改" aria-hidden="true" />
				)}
				{dropIndicator?.path === node.path &&
					dropIndicator.position !== "inside" && (
						<span
...
function TextEditorPanel({
	...
	onEditorStatusChange?: (
		path: string,
		status: FileEditorStatusSnapshot | null,
	) => void;
	onCopyPath?: (path: string) => void;
	onOpenNative?: (path: string, options?: NativeOpenOptions) => void;
	onViewFileHistory?: (path: string) => void;
	...
	const state = useReactive({
		...
		selectionToolbarLeft: 8,
		selectionToolbarTop: 8,
		selectionToolbarPlacement: "above" as "above" | "below",
	});
	...
	const markdownDecorationIdsRef = useRef<string[]>([]);
	const markdownEditorRef = useRef<TiptapEditor | null>(null);
	const metricsUpdateTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	// Ref to access handleSave without dependency issues
	const handleSaveRef = useRef<
		((options?: { notify?: boolean }) => Promise<EditorSaveResult>) | null
	>(null);
	const manualSaveRequestRef = useRef<{
		path: string;
		content: string;
		completedAt: number | null;
		notified: boolean;
		promise: Promise<EditorSaveResult>;
	} | null>(null);
	...
	apiRef.current = api;
	const loadIdentity = createEditorLoadIdentity({
		path: params?.path,
		contentVersion: params?.contentVersion,
		externalContentVersion: params?.externalContent?.version,
	});
	...
	useEffect(() => {
		...
		const pathChanged = pathRef.current !== currentParams.path;
		const hasExternalReplacement = Boolean(currentParams.externalContent);
		if (
			!pathChanged &&
			!hasExternalReplacement &&
			hasPendingEditorSave(isDirtyRef.current, currentParams.isDirty)
		) {
			// Tree/metadata versions can refresh while a user is editing. They are not
			// permission to discard the editor buffer or clear its dirty state.
			loadedContentIdentityRef.current = loadIdentity;
			return;
		}
		if (loadedContentIdentityRef.current === loadIdentity) return;
		loadedContentIdentityRef.current = loadIdentity;
		state.loading = true;
		...
		pathRef.current = currentParams.path;
		apiRef.current = api;
		...
		const readTextFile = currentParams.readFile ?? fetchFile;
		const loadContent = currentParams.externalContent
			? Promise.resolve(currentParams.externalContent.content)
			: readTextFile(currentParams.path);
		loadContent
			.then((c) => {
				if (loadedContentIdentityRef.current !== loadIdentity) return;
				if (
					...
					hasPendingEditorSave(
						isDirtyRef.current,
						paramsRef.current?.isDirty,
					)
				) {
					// A slow read must not overwrite keystrokes entered after it started.
					return;
				}
				state.content = c;
				contentRef.current = c;
				persistedContentRef.current = c;
				currentParams.onContentChange?.(currentParams.path, c);
				flushMetricsRefresh();
			})
			.catch((e) =>
				console.error("[FileTreeEditor] Failed to load file content", e),
			)
			.finally(() => {
				if (loadedContentIdentityRef.current === loadIdentity) {
					state.loading = false;
				}
			});
	}, [api, fileName, flushMetricsRefresh, loadIdentity]);

	useEffect(() => {
		if (params?.isDirty !== true || isDirtyRef.current) return;
		...
		return () => {
			for (const d of monacoDisposablesRef.current) {
				...
				if (
					typeof d === "object" &&
					d &&
					typeof (d as monacoEditor.IDisposable).dispose === "function"
				) {
	...
	useInterval(
		() => {
			if (isDirtyRef.current && pathRef.current && handleSaveRef.current) {
				void handleSaveRef.current({ notify: false });
			}
		},
		params?.autoSaveDelay && params.autoSaveDelay > 0
			? params.autoSaveDelay
			: undefined,
	);
	...
	const isMarkdown = shouldUseRichMarkdownEditor(
		params?.enableRichMarkdown,
		params?.path,
	);
	...
	const updateSelectionToolbar = useCallback(
		(editor = monacoEditorRef.current) => {
			...
			const visiblePosition = activePosition
				? editor.getScrolledVisiblePosition(activePosition)
				: null;
			if (!selectionText || !domNode || !visiblePosition) {
			...
			const selectionRect: SelectionToolbarRect = {
				left: editorRect.left + visiblePosition.left,
				top: editorRect.top + visiblePosition.top,
				right: editorRect.left + visiblePosition.left + 2,
				bottom: editorRect.top + visiblePosition.top + visiblePosition.height,
				width: 2,
			...
			const position = resolveSelectionToolbarPosition({
				selectionRect,
				toolbarSize: {
					width: toolbarRect?.width || 42,
					height: toolbarRect?.height || 38,
				},
				viewportSize: {
			...
			state.selectionText = selectionText;
			state.selectionToolbarLeft = position.left;
			state.selectionToolbarTop = position.top;
			state.selectionToolbarPlacement = position.placement;
	...
	const handleSelectionCopy = useCallback(async () => {
		...
		try {
			await writeClipboardText(state.selectionText);
			toast.success("已复制选中文本");
			hideSelectionToolbar();
		} catch (error) {
			toast.error(`复制失败: ${compactErrorMessage(formatUnknownError(error))}`);
		}
	}, [hideSelectionToolbar]);

	const handleSave = useCallback(
		async (options?: { notify?: boolean }): Promise<EditorSaveResult> => {
			const notify = options?.notify !== false;
			const currentParams = paramsRef.current;
			if (currentParams?.readOnly === true) {
				if (notify) toast.info("来源临时预览为只读，不能保存");
				return "read-only";
			}
			const currentPath = pathRef.current;
			const currentApi = apiRef.current;
			if (!currentPath || !currentApi) {
				toast.error("保存失败：当前编辑器尚未就绪");
				return "error";
			}
			// Monaco's model is the authoritative buffer at command time. React refs and
			// Dockview parameters are notification mirrors and can trail a keystroke or a
			// restored tab by one render, which previously produced a false "clean" toast.
			const modelContent = monacoEditorRef.current?.getModel()?.getValue();
			const currentContent = modelContent ?? contentRef.current;
			...
			if (
				...
				if (notify) toast.info("没有需要保存的更改");
				return "clean";
			}
			...
			try {
				...
				if (currentParams?.enableLocalSnapshots) {
				...
				if (contentRef.current !== contentToSave) {
					state.saveStatus = "dirty";
					if (notify) toast.info("文件已写入，但仍有新的修改尚未保存");
					return "stale";
				}
				isDirtyRef.current = false;
				paramsRef.current = currentParams
					? { ...currentParams, isDirty: false }
					: currentParams;
				...
				currentParams?.onContentChange?.(currentPath, contentToSave);
				currentParams?.onDirtyChange?.(currentPath, false);
				currentParams?.onAfterSave?.(currentPath);
				if (notify) toast.success(`已保存：${currentFileName}`);
				return "saved";
			} catch (e) {
				state.saveStatus = "error";
				console.error("[FileTreeEditor] Failed to save file", e);
				toast.error(`保存失败: ${compactErrorMessage(formatUnknownError(e))}`);
				return "error";
			}
		},
		[flushMetricsRefresh, isMarkdown],
	);
	...
	const handleManualSave = useCallback(() => {
		const requestIdentity = {
			...
			content:
				monacoEditorRef.current?.getModel()?.getValue() ?? contentRef.current,
		};
		const previousRequest = manualSaveRequestRef.current;
		if (
			previousRequest?.path === requestIdentity.path &&
			previousRequest?.content === requestIdentity.content &&
			(previousRequest.completedAt === null ||
				Date.now() - previousRequest.completedAt < 250)
		) {
		...
		manualSaveRequestRef.current = request;
	...
	const requestManualSave = useCallback(() => {
		const promise = handleManualSave();
		void promise.then(() => {
			const request = manualSaveRequestRef.current;
			if (request?.promise !== promise || request.notified) return;
	...
	const handleChange = (v: string | undefined) => {
		...
		const hasBufferChanges = hasEditorBufferChanged(
			c,
			persistedContentRef.current,
		);
		if (hasBufferChanges && !isDirtyRef.current) {
			isDirtyRef.current = true;
			state.saveStatus = "dirty";
		...
		} else if (hasBufferChanges && state.saveStatus !== "dirty") {
			state.saveStatus = "dirty";
		} else if (!hasBufferChanges && isDirtyRef.current) {
			isDirtyRef.current = false;
			state.saveStatus = "saved";
			api.setTitle(fileName);
			api.updateParameters({
			...
			params?.onDirtyChange?.(params.path, false);
		}
	};

	const handleMount: import("@monaco-editor/react").EditorProps["onMount"] = (
		editor,
		monaco,
	) => {
		...
		const refreshMarkdownDecorations = () => {
			const model = editor.getModel();
			markdownDecorationIdsRef.current = editor.deltaDecorations(
				markdownDecorationIdsRef.current,
				languageId === "markdown" && model
					? buildMarkdownSourceDecorations(model.getValue()).map(
							(decoration) => ({
								range: {
									startLineNumber: decoration.startLineNumber,
									startColumn: decoration.startColumn,
									endLineNumber: decoration.endLineNumber,
									endColumn: decoration.endColumn,
								},
								options: {
			...
			const domNode = editor.getDomNode();
			if (domNode) {
				domNode.dataset.markdownDecorationCount = String(
					markdownDecorationIdsRef.current.length,
				);
			}
		};
		refreshMarkdownDecorations();
		monacoDisposablesRef.current.push(
			editor.onDidChangeModelContent(refreshMarkdownDecorations),
		);
		...
		if (params?.initialViewState && params.path) {
			try {
				editor.restoreViewState(
					params.initialViewState as monacoEditor.editor.ICodeEditorViewState,
				);
			} catch {
		...
		const minimapId = editor.addCommand(
			monacoEditor.KeyMod.CtrlCmd |
				monacoEditor.KeyMod.Shift |
				monacoEditor.KeyCode.KeyM,
			() => {
				const newValue = !editor.getOption(monacoEditor.editor.EditorOption.minimap)
					.enabled;
		...
		const wordWrapId = editor.addCommand(
			monacoEditor.KeyMod.Alt | monacoEditor.KeyCode.KeyZ,
			() => {
				const newValue =
					editor.getOption(monacoEditor.editor.EditorOption.wordWrap) === "on"
						? "off"
						: "on";
		...
		const gotoId = editor.addCommand(
			monacoEditor.KeyMod.CtrlCmd | monacoEditor.KeyCode.KeyG,
			() => {
		...
		const findId = editor.addCommand(
			monacoEditor.KeyMod.CtrlCmd | monacoEditor.KeyCode.KeyF,
			() => {
		...
		const formatId = editor.addCommand(
			monacoEditor.KeyMod.Shift |
				monacoEditor.KeyMod.Alt |
				monacoEditor.KeyCode.KeyF,
			() => {
		...
		const replaceId = editor.addCommand(
			monacoEditor.KeyMod.CtrlCmd | monacoEditor.KeyCode.KeyH,
			() => {
		...
		monacoDisposablesRef.current.push(
			editor.onDidChangeCursorPosition((e) => {
				if (
					state.cursorPosition.line === e.position.lineNumber &&
					state.cursorPosition.column === e.position.column
				) {
					return;
				}
		...
		monacoDisposablesRef.current.push(
			editor.onDidChangeCursorSelection((e) => {
				...
				// Do not depend on WKWebView painting Monaco's dynamic overlay sheet.
				// An inline model decoration makes the selected glyph range visible in the
				// same DOM/token layer as the text in both editable and read-only previews.
						...
						: [
								{
									...
									options: {
										inlineClassName:
											"file-tree-editor-user-selection",
										inlineClassNameAffectsLetterSpacing: false,
									},
								},
							],
				...
				const domNode = editor.getDomNode();
				if (domNode) {
					domNode.dataset.selectionDecorationCount = String(
						selectionDecorationIdsRef.current.length,
					);
				}

				const nextSelectionLength = selection.isEmpty()
					? 0
		...
		monacoDisposablesRef.current.push(
			editor.onDidScrollChange(() => updateSelectionToolbar(editor)),
			editor.onDidLayoutChange(() => updateSelectionToolbar(editor)),
		);
		const handleViewportResize = () => updateSelectionToolbar(editor);
		...
		state.minimapEnabled = editor.getOption(
			monacoEditor.editor.EditorOption.minimap,
		).enabled;
	...
	const editorLoading = state.loading;
	useEffect(() => {
		if (editorLoading || !params?.line || isMarkdown) return;
		...
		editor.focus();
	}, [editorLoading, isMarkdown, params?.line]);

	useEffect(() => {
		if (editorLoading || isMarkdown) return;
		const editor = monacoEditorRef.current;
	...
	}, [editorLoading, isMarkdown, params?.searchQuery]);

	const runMarkdownCommand = useCallback(
		(command: string, options?: { allowActivePanel?: boolean }) => {
			const root = focusRef.current;
			const allowActivePanel = options?.allowActivePanel === true && api.isActive;
			if (
				!root ||
				(!allowActivePanel && !root.contains(document.activeElement as Node))
			)
				return false;
			...
			const inMonaco =
				allowActivePanel || !!activeElement?.closest(".monaco-editor");
			const inProseMirror =
				allowActivePanel || !!activeElement?.closest(".ProseMirror");
			...
			if (inMonaco && monacoEditor) {
				switch (command) {
					case "undo":
						...
						return true;
					case "redo":
						monacoEditor.trigger("native-menu", "redo", null);
						return true;
					case "copy":
	...
	// Listen for explicit single-panel saves and save-all separately. A plain
	// Cmd/Ctrl+S is routed here with the active Dockview path, preventing a stale
	// restored panel instance from answering for the visible tab.
	useEffect(() => {
		const handleBrowserSave = (event: Event) => {
			const detail = (event as CustomEvent<FileEditorSaveDetail>).detail;
			if (detail?.scope && detail.scope !== params?.commandScope) return;
			if (!detail?.path || detail.path !== pathRef.current) return;
			requestManualSave();
		};
		...
		document.addEventListener(FILE_EDITOR_SAVE_EVENT, handleBrowserSave);
	...
	}, [params?.commandScope, requestManualSave]);
	...
	}, [api, editorLoading, requestManualSave, runMarkdownCommand]);
	...
	const metricsVersion = state.metricsVersion;
	const contentMetrics = useMemo(() => {
		void metricsVersion;
		return getContentMetrics(contentRef.current);
	}, [metricsVersion]);
	const statusLanguageLabel = isMarkdown
		? "Markdown"
		: getLanguageDisplayName(languageId);
	const onEditorStatusChange = params?.onEditorStatusChange;
	const editorSaveStatus = state.saveStatus;
	const editorLastSavedAt = state.lastSavedAt;
	const editorCursorLine = state.cursorPosition.line;
	const editorCursorColumn = state.cursorPosition.column;
	const editorSelectionLength = state.selectionLength;
	const editorTabSize = state.indentation.tabSize;
	const editorInsertSpaces = state.indentation.insertSpaces;
	const editorEol = state.eol;

	useEffect(() => {
		if (editorLoading || !editorPath) return;
						...
						<TooltipTrigger asChild>
							<button
								...
								className={cn(
									...
									state.wordWrapEnabled && "is-active",
								)}
							>
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<button
								...
								className={cn(
									"file-tree-editor-iconbutton",
									state.minimapEnabled && "is-active",
								)}
							>
			...
			<Tooltip>
				<TooltipTrigger asChild>
					...
					>
						...
						{state.saveStatus === "dirty" && (
							<span className="file-tree-editor-button-dot" aria-hidden="true" />
						)}
					</button>
				</TooltipTrigger>
				<TooltipContent>
		...
		<EditorPanelFrame path={params?.path}>
			<div className="flex h-full min-w-0 flex-col overflow-hidden">
				...
				<div
					...
					className="min-h-0 min-w-0 flex-1 overflow-hidden outline-none"
					onPointerDown={(event) => {
						// Monaco owns text focus, caret, selection and native undo.
						// Only focus this shortcut scope when the actual wrapper
						// background is clicked; bubbled editor events must not steal it.
						if (event.target === event.currentTarget) {
							focusRef.current?.focus();
						}
					}}
				>
					{state.selectionToolbarVisible && state.selectionText ? (
						<div
							ref={selectionToolbarRef}
							role="toolbar"
							aria-label="文本选择操作"
							data-placement={state.selectionToolbarPlacement}
							className="fixed z-[3000] flex items-center rounded-[8px] border border-border bg-background/95 px-1.5 py-1 shadow-lg backdrop-blur"
							style={{
								left: state.selectionToolbarLeft,
								top: state.selectionToolbarTop,
							}}
							onPointerDown={(event) => event.preventDefault()}
						>
							<button
								type="button"
								className="flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/40"
					...
					<CodeEditor
						value={contentRef.current}
						language={languageId}
						hostOwnedCommandIds={["editor.save"]}
						onChange={handleChange}
						onMount={handleMount}
						options={{
							readOnly,
							// WKWebView exposes a partial EditContext implementation on some
							// macOS releases. Monaco's textarea path is more reliable there and
							// restores the real caret/selection geometry.
							editContext: false,
							...
							// Monaco injects its menu stylesheet dynamically. WKWebView can
							// render that menu before those rules are available, producing a
							// raw, unusable list. Let the platform text context menu handle
							// copy/cut/paste instead; keyboard and command-palette actions
							// remain registered with Monaco.
							contextmenu: false,
						}}
					/>
				</div>
...
export function FileTreeEditor({
	...
	const showExplorerSearchFilterInput =
		showExplorerSearchFilter ?? !isVsCodeVariant;
	...
	const staticTreeResult = useMemo(
		() =>
			rootPath && isStaticFileSource
				? buildStaticFilesTree(rootPath, staticFiles ?? [])
				: null,
	...
	const staticFileContentMap = useMemo(
		() =>
			rootPath && isStaticFileSource
				? buildStaticFileContentMap(rootPath, staticFiles ?? [])
				: null,
		[isStaticFileSource, rootPath, staticFiles],
	);
		...
		() =>
			isStaticFileSource
				? (staticFiles ?? [])
						.map(
							(file) =>
								`${normalizeStaticFilePath(file.path)}:${file.size ?? getUtf8ByteSize(file.content)}`,
						)
						.join("|")
				: undefined,
	...
	const readTextFile = useCallback(
		async (path: string) => {
			...
			if (content === undefined) {
				throw new Error(
					`模板文件不存在: ${getWorkspaceRelativePath(rootPath, path)}`,
				);
			}
			return content;
		},
	...
	const editorStatusByPathRef = useRef<Map<string, FileEditorStatusSnapshot>>(
		new Map(),
	);
	...
	const getEditorSessionSnapshot = useCallback((): EditorSessionState => {
		...
		return {
			...
			openFiles: panels.map((panel) => ({
				path: panel.id,
				pinned:
					pinnedPathsRef.current.has(panel.id) ||
					!(panel.params as { isPreview?: boolean } | undefined)?.isPreview,
			})),
		...
		(reason: string) => {
			...
			for (const delay of delays) {
				const timeoutId = window.setTimeout(() => {
					...
					dockviewLayoutTimeoutsRef.current =
						dockviewLayoutTimeoutsRef.current.filter((id) => id !== timeoutId);
				}, delay);
	...
	const handleEditorContentReport = useCallback(
	...
	const handleEditorStatusChange = useCallback(
		(path: string, status: FileEditorStatusSnapshot | null) => {
			...
			if (
				previous &&
				getEditorStatusSignature(previous) === getEditorStatusSignature(status)
			) {
				return;
			}
	...
	const waitForDirtyFilesToSave = useCallback(
		(paths: string[], timeoutMs = 6000) => {
			...
			return new Promise<string[]>((resolve) => {
				...
				const interval = window.setInterval(() => {
					const remaining = pendingPaths.filter((path) =>
						dirtyFilesRef.current.has(path),
					);
	...
	const saveAllDirty = useCallback(async () => {
		...
		toast.error(
			`仍有 ${remainingPaths.length} 个文件未保存，请检查保存失败的编辑器`,
		);
		return false;
	...
	useEffect(() => {
		const handlePointerDown = (event: PointerEvent) => {
			...
			treeShortcutScopeRef.current = !!(
				target && treeFocusRef.current?.contains(target)
			);
		};
	...
	const confirmDiscardDirtyPaths = useCallback(
		async (dirtyPaths: string[], actionLabel: string) => {
			...
			const preview = uniquePaths
				...
				.join("、");
			const suffix =
				uniquePaths.length > 3 ? ` 等 ${uniquePaths.length} 个文件` : "";
			return askConfirm(
	...
	const confirmDiscardDirtyPath = useCallback(
		(path: string, actionLabel: string) =>
			confirmDiscardDirtyPaths(getDirtyPathsUnderPath(path), actionLabel),
		...
		async ({
			...
			if (staticTreeResult) {
				...
				try {
					...
					if (force || treeSignatureRef.current !== nextSignature) {
						...
						state.tree = nextTree;
						state.selectedPaths = state.selectedPaths.filter((path) =>
							nextNodeByPath.has(path),
						);
						state.selectedPath =
							state.selectedPath && nextNodeByPath.has(state.selectedPath)
								? state.selectedPath
								: null;
					}
				} finally {
			...
			try {
				...
				if (!nextTree.children?.length) {
					const rootExists = await workspaceApi
						.fileExists(rootPath)
						.catch(() => true);
					if (!rootExists) {
				...
				if (force || treeSignatureRef.current !== nextSignature) {
					...
					state.tree = nextTree;
					{
						const next = state.selectedPaths.filter((path) =>
							nextNodeByPath.has(path),
						);
				...
				if (!silent && nextResult.partialLoadErrorCount > 0) {
					toast.warning(
						`文件树已加载，但有 ${nextResult.partialLoadErrorCount} 个目录读取失败`,
					);
				}
			} catch (e) {
	...
	const workspaceTree = state.tree;
	const workspaceSelectedPaths = state.selectedPaths;
	const workspaceDirtyVersion = state.dirtyVersion;
	const workspaceActiveFile = state.activeFile;
	const workspaceLoading = state.loading;
	const workspaceTreeLoadError = state.treeLoadError;
	const workspacePartialLoadErrorCount = state.partialLoadErrorCount;
	const workspaceEditorTabsVersion = state.editorTabsVersion;
	const workspaceGitStatus = state.gitStatus;
	const workspaceGitStatusError = state.gitStatusError;
	const workspaceGitStatusLoading = state.gitStatusLoading;
	const workspaceSidebarCollapsed = state.sidebarCollapsed;
	const workspaceSidebarPanel = state.sidebarPanel;
	const workspaceExplorerSearchQuery = state.explorerSearchQuery;
	const workspaceOpenNodes = state.openNodes;
	const workspaceCollapseAllVersion = state.collapseAllVersion;
	const workspaceDragSource = state.dragSrc;
	const workspaceDropTarget = state.dropTarget;
	const workspaceDropIndicator = state.dropIndicator;
	const workspaceRenamingPath = state.renamingPath;
	const workspaceCutPaths = state.cutPaths;
	const workspaceInlineCreate = state.inlineCreate;
	const workspaceAutoSaveDelayMs = state.autoSaveDelayMs;
	const workspaceRecentFiles = state.recentFiles;
	const treeStats = useMemo(() => getTreeStats(workspaceTree), [workspaceTree]);
	const nodeByPath = useMemo(
		() => buildTreeNodeIndex(workspaceTree),
		[workspaceTree],
	);
	const selectedPathSet = useMemo(
		() => new Set(workspaceSelectedPaths),
		[workspaceSelectedPaths],
	);
	const dirtyPathSet = useMemo(() => {
		void workspaceDirtyVersion;
		return new Set(dirtyFilesRef.current);
	}, [workspaceDirtyVersion]);
	const refreshGitStatus = useCallback(async () => {
		...
		try {
			...
			state.gitStatusError =
				error instanceof Error ? error.message : "Git 状态读取失败";
		} finally {
	...
	}, [refreshGitStatus, workspaceDirtyVersion]);

	const activeEditorStatus = useMemo(() => {
		void editorStatusVersion;
		return workspaceActiveFile
			? (editorStatusByPathRef.current.get(workspaceActiveFile) ?? null)
			: null;
	}, [editorStatusVersion, workspaceActiveFile]);

	const activeFileType = useMemo(
		() => (workspaceActiveFile ? getFileType(workspaceActiveFile) : null),
		[workspaceActiveFile],
	);
	...
	useEffect(() => {
		...
		void workspaceDirtyVersion;
		void workspaceEditorTabsVersion;
		onStateChange({
			rootPath,
			loading: workspaceLoading,
			treeLoadError: workspaceTreeLoadError,
			partialLoadErrorCount: workspacePartialLoadErrorCount,
			activeFile: workspaceActiveFile,
			selectedPaths: [...workspaceSelectedPaths],
			...
			gitChangeCount:
				(workspaceGitStatus?.files.length ?? 0) + dirtyFilesRef.current.size,
			gitStatusLoading: workspaceGitStatusLoading,
			gitStatusError: workspaceGitStatusError,
			activeFileType,
			...
			sidebarPanel: workspaceSidebarPanel,
		});
	}, [
		activeEditorStatus,
		activeFileType,
		...
		workspaceActiveFile,
		workspaceDirtyVersion,
		workspaceEditorTabsVersion,
		workspaceGitStatus,
		workspaceGitStatusError,
		workspaceGitStatusLoading,
		workspaceLoading,
		workspacePartialLoadErrorCount,
		workspaceSelectedPaths,
		workspaceSidebarCollapsed,
		workspaceSidebarPanel,
		workspaceTreeLoadError,
	]);
	...
	const getVisibleNodes = useCallback((): FsNode[] => {
		...
		const tree = filterActive
			? filterTreeByQuery(state.tree, state.explorerSearchQuery)
			: state.tree;
		const visit = (node: FsNode) => {
			result.push(node);
			if (
				node.is_dir &&
				(filterActive || state.openNodes.has(node.path)) &&
				node.children
			) {
				for (const child of node.children) {
					visit(child);
	...
	const handleNavigateTree = useCallback(
		(direction: "up" | "down") => {
			...
			const currentIndex = state.selectedPath
				? visibleNodes.findIndex((node) => node.path === state.selectedPath)
				: -1;
	...
	const handleShiftArrow = useCallback(
		(direction: "up" | "down") => {
			...
			const currentIndex = state.selectedPath
				? visibleNodes.findIndex((n) => n.path === state.selectedPath)
				: -1;
	...
	const revealInExplorer = useCallback(
		...
		[activateTreeScope, rootPath, selectSinglePath],
	);
	...
	const getSelectionItems = useCallback(
		...
		[nodeByPath, selectedPathSet],
	);

	const getUniquePath = useCallback(
		async (destDir: string, originalName: string) => {
			...
			while (true) {
				...
				index += 1;
				candidateName =
					index === 1 ? `${stem} copy${ext}` : `${stem} copy ${index}${ext}`;
			}
		},
		[],
	);

	const copyDirRecursive = useCallback(
		async (srcDir: string, destDir: string): Promise<void> => {
	...
	const getTrashPath = useCallback(
		async (path: string) => {
			const trashRoot = joinWorkspacePath(
				rootPath || getParentPath(path),
				".shuan-os-trash",
			);
	...
	useEffect(() => {
		const session = persistSession
			? readEditorSession(rootPath ?? undefined)
			: null;
		state.tree = null;
		state.treeLoadError = null;
		state.partialLoadErrorCount = 0;
	...
	useEffect(() => {
		...
		const setupWatcher = async () => {
			...
			unlistenFileChange = await listen<{ path: string }>(
				"file-change",
				async (event) => {
					...
					try {
						...
						const localContent =
							currentEditorContentRef.current.get(changedPath) ?? originalContent;
	...
	const handleExternalReload = useCallback(async () => {
		...
		if (!dialog) return;
		try {
			const latestContent = await workspaceApi
				.readFile(dialog.path)
				.catch(() => dialog.newContent);
			replaceEditorPanelContent(dialog.path, latestContent);
			state.externalChangeDialog = null;
			toast.success("已重新加载外部版本");
		} catch (error) {
			toast.error(
				`重新加载失败: ${compactErrorMessage(formatUnknownError(error))}`,
			);
		}
	...
	const handleExternalKeepLocal = useCallback(async () => {
		...
		if (!dialog) return;
		const localContent =
			currentEditorContentRef.current.get(dialog.path) ?? dialog.localContent;
		try {
			...
			toast.error(
				`保留本地失败: ${compactErrorMessage(formatUnknownError(error))}`,
			);
		}
	}, [replaceEditorPanelContent]);
	...
	const recordRecentFile = useCallback((path: string) => {
		state.recentFiles = [
			path,
			...state.recentFiles.filter((item) => item !== path),
		].slice(0, 30);
	}, []);
	...
	const resolveFileComponent = useCallback(
		(path: string): FileType =>
			openAsTextRef.current.has(path) ? "text" : getFileType(path),
		[],
	);
	...
	const editorInstancesRef = useRef<
		Map<string, monacoEditor.editor.IStandaloneCodeEditor>
	>(new Map());
			...
			try {
				const panel = dockviewRef.current.api.addPanel({
					...
					params: {
						...
						isPreview,
						isPinned,
						autoSaveDelay: manualSaveOnly ? 0 : state.autoSaveDelayMs,
						commandScope,
						...
						onViewFileHistory: enableLocalSnapshots
					...
					position:
						groups.length > 0
							? { referenceGroup: groups[0], direction: "within" }
							: undefined,
				});
	...
	const handleOpenWikiLink = useCallback(
		(repoRelativePath: string) => {
			...
			const fullPath = rootPath
				? joinWorkspacePath(rootPath, repoRelativePath)
				: repoRelativePath;
	...
	const handleEnterOpen = useCallback(() => {
		if (state.selectedPath) {
			...
			if (node && !node.is_dir) {
				...
				handleOpenChange(
					state.selectedPath,
					!state.openNodes.has(state.selectedPath),
				);
			}
		}
	}, [handleFileClick, handleOpenChange, nodeByPath]);
	...
	const handleRenameSnapshot = useCallback(
		async (snapshot: LocalFileSnapshot) => {
			...
			try {
				const snapshots = await updateLocalSnapshotLabel(
					path,
					snapshot.id,
					label.trim(),
				);
	...
	const handleRestoreSnapshot = useCallback(
		async (snapshot: LocalFileSnapshot) => {
			const path =
				state.snapshotCompareDialog?.path ??
				state.snapshotPreviewDialog?.path ??
				state.fileHistoryDialog?.path;
			if (!path) return;
			const ok = await askConfirm(
	...
	const pasteClipboardItems = useCallback(
		async (destinationPath?: string) => {
			...
			if (clipboard.mode === "cut") {
				const dirtyPaths = clipboard.items.flatMap((item) =>
					getDirtyPathsUnderPath(item.path),
				);
	...
	const handleTreeDocumentKeyDown = useCallback(
		(event: KeyboardEvent) => {
			...
			if (!shortcut) return;
			if (
				!isEditorScopeActive(event) ||
				!isTreeScopeActive(event) ||
			...
			if (shortcut === "paste") {
				...
				const selectedNode = state.selectedPath
					? nodeByPath.get(state.selectedPath)
					: null;
	...
	useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) =>
			handleTreeDocumentKeyDownRef.current(event);
	...
	const handleCreateFile = useCallback(
		...
		[assertWritable, rootPath, selectSinglePath],
	);
		...
		async (name: string) => {
			...
			if (type === "file") {
				...
				try {
					...
					toast.error(
						`新建文件夹失败: ${compactErrorMessage(formatUnknownError(e))}`,
					);
				}
			}
		},
	...
	const handleRename = useCallback(
		async (path: string, isDir: boolean) => {
			...
			const canDiscardDirty = await confirmDiscardDirtyPath(
				path,
				`重命名 "${oldName}"`,
			);
	...
	const handleDeleteSelection = useCallback(async () => {
		...
		try {
			...
			selectSinglePath(rootPath);
			toast.success(
				items.length === 1 ? "已删除" : `已删除 ${items.length} 个项目`,
			);
		} catch (e) {
	...
	const handleMove = useCallback(
		async (srcPath: string, srcIsDir: boolean, destDir: string) => {
			...
			if (srcIsDir && (destDir === srcPath || destDir.startsWith(`${srcPath}/`)))
				return;
			const canDiscardDirty = await confirmDiscardDirtyPath(
				srcPath,
				`移动 "${name}"`,
	...
	const showExplorerMoreActions = useCallback(
		(event: React.MouseEvent<HTMLButtonElement>) => {
			...
			const visibleItems = normalizeMenuItems(
				explorerMoreActions,
	...
	const explorerPaneParams = useMemo<ExplorerPaneParams>(
		() => ({
			...
			selectedPaths: workspaceSelectedPaths,
			selectedPathSet,
			searchQuery: workspaceExplorerSearchQuery,
			totalFiles: treeStats.files,
			totalFolders: treeStats.folders,
			openNodes: workspaceOpenNodes,
			autoExpandDepth,
			collapseAllVersion: workspaceCollapseAllVersion,
			showSearchFilter: showExplorerSearchFilterInput,
			showFooter: !isVsCodeVariant,
			dirtyPaths: dirtyPathSet,
			dragSrc: workspaceDragSource,
			dropTarget: workspaceDropTarget,
			dropIndicator: workspaceDropIndicator,
			treeFocusRef,
			emptyAreaItems,
			...
			renamingPath: workspaceRenamingPath,
			cutPaths: workspaceCutPaths,
			...
			inlineCreate: workspaceInlineCreate,
		...
		[
			...
			dirtyPathSet,
			workspaceActiveFile,
			workspaceCollapseAllVersion,
			workspaceCutPaths,
			workspaceDragSource,
			workspaceDropIndicator,
			workspaceDropTarget,
			workspaceExplorerSearchQuery,
			workspaceInlineCreate,
			workspaceLoading,
			workspaceOpenNodes,
			workspacePartialLoadErrorCount,
			workspaceRenamingPath,
			workspaceSelectedPaths,
			workspaceTree,
			workspaceTreeLoadError,
		],
	...
	useEffect(() => {
		...
		if (!workspaceTree || workspaceLoading) return;
	...
	}, [isVsCodeVariant, workspaceLoading, workspaceTree]);
		...
		try {
			const panel = dockviewRef.current.api.addPanel({
				...
				params: {
					...
					...(pendingFile.searchQuery
						? { searchQuery: pendingFile.searchQuery }
						: {}),
					isPreview: !!pendingFile.preview,
					isPinned: !!pendingFile.pinned,
					autoSaveDelay: manualSaveOnly ? 0 : workspaceAutoSaveDelayMs,
					commandScope,
					enableLocalSnapshots,
					enableRichMarkdown,
					assetId,
	...
	const focusSidebarSearchInput = useCallback(() => {
		...
		requestAnimationFrame(() => {
			const input = rootContainerRef.current?.querySelector<HTMLInputElement>(
				isVsCodeVariant
					? ".file-tree-vscode-search-panel input"
					: ".file-tree-explorer-search-input",
			);
			input?.focus();
			input?.select();
		});
	...
	useEffect(() => {
		if (!openFileRequest || workspaceLoading || !workspaceTree) return;
		if (lastOpenFileRequestRef.current === openFileRequest.nonce) return;
		lastOpenFileRequestRef.current = openFileRequest.nonce;
		handleFileClick(openFileRequest.path, {
			preview: openFileRequest.preview,
			pinned: openFileRequest.pinned ?? !openFileRequest.preview,
		});
	}, [handleFileClick, openFileRequest, workspaceLoading, workspaceTree]);
	...
	const runActiveEditorAction = useCallback(
		(actionId: string, emptyHint: string) => {
			const activeId = dockviewRef.current?.api.activePanel?.id;
			const editor = activeId
				? editorInstancesRef.current.get(activeId)
				: undefined;
			if (!editor) {
	...
	const quickOpenFiles = useMemo<QuickOpenFile[]>(() => {
		...
		const rank = new Map(
			workspaceRecentFiles.map((path, index) => [path, index]),
		);
		files.sort((a, b) => {
			...
			return ra !== rb
				? ra - rb
				: `${a.dir}/${a.label}`.localeCompare(`${b.dir}/${b.label}`);
		});
		return files;
	}, [nodeByPath, quickOpenOpen, rootPath, workspaceRecentFiles]);
	const paletteCommands = useMemo<WorkspaceCommand[]>(() => {
		const list: WorkspaceCommand[] = [
			...
			},
			{
				...
				title: "源代码管理",
				group: "导航",
				run: () => showSidebarPanel("sourceControl"),
			},
			{
				id: "git-refresh",
				title: "刷新 Git 状态",
			...
			},
			{
				...
				keybinding: "mod+g",
				run: () =>
					runActiveEditorAction("editor.action.gotoLine", "请先打开一个可编辑文件"),
			},
			{
				id: "toggle-sidebar",
				title: "切换侧栏",
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			if (shortcut === "save-all" && editableTarget) {
				const activePath = dockviewRef.current?.api.activePanel?.id;
				if (activePath && editorInstancesRef.current.has(activePath)) {
					event.preventDefault();
					event.stopPropagation();
		...
		// Capture plain save before Monaco/WebKit can translate it into a generic
		// editor command. The active Dockview panel remains the single target.
	...
	const sourceControlChangeCount =
		(state.gitStatus?.files.length ?? 0) + dirtyPathSet.size;
	...
	const fullWidthBoard =
		state.sidebarPanel === "board" && Boolean(extraSidebarPane);
	const fullWidthDebug =
		state.sidebarPanel === "debug" && Boolean(debugSidebarPane);
	const activeCustomSidebarPane = state.sidebarPanel.startsWith("custom:")
		? customSidebarPanes?.find(
				(pane) => toCustomSidebarPanelId(pane.id) === state.sidebarPanel,
			)
		: undefined;
	const fullWidthCustomPane = Boolean(activeCustomSidebarPane?.fullWidth);
	const fullWidthActivityPane =
		fullWidthBoard || fullWidthDebug || fullWidthCustomPane;
		...
		const Watermark = () => {
				...
				return (
					<div className="file-tree-editor-watermark is-vscode-watermark">
						...
						{panelLayout.mode === "compact" ? null : (
							<div
								className="file-tree-editor-vscode-watermark-shortcuts"
								aria-hidden="true"
							>
			...
			return (
				<div className="file-tree-editor-watermark">
					...
					<div className="file-tree-editor-watermark-description">
						从左侧资源管理器选择文件。
					</div>
		...
		>
              ...
              caused "Previous layout not found for panel index -1"). */}
					...
					>
						...
						<ResizablePanel
							...
							order={1}
							defaultSize={
								fullWidthActivityPane ? 100 : panelLayout.sidebarDefaultSize
							}
						...
						>
							{isVsCodeVariant ? (
								<div className="file-tree-vscode-sidebar-shell">
									<nav
										className="file-tree-vscode-activitybar"
										aria-label="工作区活动栏"
									>
										<VscodeTooltipButton
									...
									<div className="file-tree-vscode-sidebar-panel">
										<div className="file-tree-vscode-sidebar-titlebar">
											...
											{state.sidebarPanel === "explorer" && (
												...
												>
													{!state.loading && (
														<span className="file-tree-vscode-sidebar-chip">
															{treeStats.files}
														</span>
													)}
													{dirtyPathSet.size > 0 && (
														<span className="file-tree-vscode-sidebar-chip is-dirty">
															{dirtyPathSet.size} 未保存
														</span>
													)}
													{state.partialLoadErrorCount > 0 && (
											...
											extraSidebarPane?.titlebarActionsPortalId ? (
												<div
											...
											overviewSidebarPane?.titlebarActionsPortalId ? (
												<div
										...
										{state.sidebarPanel === "board" && extraSidebarPane ? (
											...
											<ExplorerPane
												...
												api={
													null as unknown as import("@/desktop/components/dockview-core").PaneviewPanelApi
												}
												containerApi={
													null as unknown as import("@/desktop/components/dockview-core").PaneviewApi
												}
												title="资源管理器"
											/>
										) : (
											<div className="file-tree-vscode-search-panel">
												<GlobalSearch
													...
													onFileClick={(path, line) =>
														handleFileClick(path, { line, pinned: true })
													}
												/>
											</div>
										)}
								...
								<div className="h-full bg-muted/[0.03]">
									<PaneviewReact
										...
										onReady={(event) => {
											...
											const cachedLayout = readCachedPaneviewLayout(
												rootPath ?? undefined,
												"sidebar",
											);
											if (cachedLayout) {
											...
											if (!event.api.getPanel("explorer")) {
												...
												updatePanelParametersIfChanged(
													event.api.getPanel("explorer"),
													explorerPaneParams,
												);
											}
						...
						{!fullWidthActivityPane && (
							<ResizableHandle
								className={
									isVsCodeVariant ? "file-tree-vscode-resize-handle" : undefined
								}
							/>
						)}
								...
								<div className="relative flex h-full min-w-0 flex-col overflow-hidden">
									...
									>
										<DockviewReact
											onReady={(event) => {
													...
													for (const file of session.openFiles) {
														...
														const panel = event.api.addPanel({
															...
															params: {
																...
																onEditorStatusChange: handleEditorStatusChange,
																onCopyPath: handleCopyPath,
																onOpenNative: supportsFileSystemShell
																	? handleOpenNative
																	: undefined,
																onViewFileHistory: enableLocalSnapshots
																	? handleViewFileHistory
												...
												if (pendingFileRef.current) {
													...
													const panel = event.api.addPanel({
														...
														autoSaveDelay: manualSaveOnly ? 0 : state.autoSaveDelayMs,
															...
															onPinRequest: pinEditorPanel,
														onViewStateChange: handleEditorViewStateChange,
														initialViewState: editorMementosRef.current.get(
															pendingFile.path,
														),
														onEditorInstance: setEditorInstance,
														},
												...
												const removePanelDisposable = event.api.onDidRemovePanel(
													(panel) => {
														...
														if (closedTabsRef.current.length > 25)
												...
												const closePanelsSafely = (
													panels: Array<(typeof event.api.panels)[number]>,
												) => {
												...
												const handleTabCtx = (e: MouseEvent) => {
													...
													if (!panelId && tab) {
														...
														if (tabContent) {
															const title = tabContent.textContent
																?.trim()
																.replace(/\s+/g, " ");
															if (title) {
																const panel = event.api.panels.find(
																	(p) =>
																		p.title === title || p.title === title.replace(/\s+/g, " "),
																);
													...
													if (panelId) {
														...
														const targetIsPreview = !!(
															targetPanel?.params as { isPreview?: boolean } | undefined
														)?.isPreview;
														...
														if (isVsCodeVariant) {
															items = [
																...
																separatorItem,
																{
																	...
																	onClick: () =>
																		closePanelsSafely(
																			allPanels.filter((panel) => panel.id === panelId),
																		),
											...
											defaultTabComponent={
												FileTab as React.FunctionComponent<IDockviewPanelHeaderProps>
											}
			...
			{state.ctxMenu && (
				<ContextMenuPortal
					...
					portalContainer={
						rootContainerRef.current?.closest<HTMLElement>("[role='dialog']") ?? null
					}
				/>
			)}
