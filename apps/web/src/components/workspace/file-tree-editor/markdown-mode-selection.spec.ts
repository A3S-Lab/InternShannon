...
import { shouldUseRichMarkdownEditor, sourceModeContentForSave } from "./markdown-mode-selection.ts";
...
const skillsPageSource = readFileSync(
  fileURLToPath(new URL("../../../desktop/pages/agent/SkillsPage.tsx", import.meta.url)),
  "utf8",
);
...
test("SkillsPage wires its source-mode contract into FileTreeEditor", () => {
  assert.match(skillsPageSource, /enableRichMarkdown=\{SKILL_PANEL_ENABLE_RICH_MARKDOWN\}/);
  assert.match(fileTreeEditorSource, /shouldUseRichMarkdownEditor\(params\?\.enableRichMarkdown, params\?\.path\)/);
  assert.match(fileTreeEditorSource, /if \(isMarkdown\)[\s\S]*?<MarkdownEditor/);
  assert.equal(shouldUseRichMarkdownEditor(false, "users/local/skills/excalidraw/SKILL.md"), false);
});
...
test("source-mode SKILL.md save preserves YAML frontmatter byte for byte", () => {
  ...
  assert.match(fileTreeEditorSource, /sourceModeContentForSave\(contentRef\.current\)/);
});
