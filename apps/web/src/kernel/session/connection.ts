...
import {
	autoDecideAuthorization,
	recordAuthorizationDecision,
} from "@/lib/hitl-auth";
import { recordInternShannonMemoryEvent } from "@/lib/internShannon-memory-timeline";
import { normalizeKnowledgeSourceReferences } from "@/lib/knowledge-citation";
...
} from "./session-runtime-busy-state";
import { normalizeSessionStatusPatch } from "./session-status-normalization";
import { SessionStatusRequestRegistry } from "./session-status-request-state";
import {
...
const sessionStatusRequests = new SessionStatusRequestRegistry();
let sessionStatusRequestSequence = 0;
const activeContextCompactionNotices = new Map<
	string,
...
const diffCache = new Map<
	string,
	Map<string, { before?: string; after?: string; filePath?: string }>
>();
...
const toolOutputEncoder =
	typeof TextEncoder !== "undefined" ? new TextEncoder() : null;

function utf8ByteLength(text: string): number {
	return toolOutputEncoder
		? toolOutputEncoder.encode(text).length
		: text.length;
}
...
function appendToolOutputForUi(
...
function startContextCompactionNotice(sessionId: string): void {
	if (activeContextCompactionNotices.has(sessionId)) return;
	const notice = {
		id: `context-compaction-${nextMsgId()}`,
		timestamp: Date.now(),
	};
	activeContextCompactionNotices.set(sessionId, notice);
	agentModel.upsertMessage(sessionId, {
		id: notice.id,
		role: "system",
		content: "正在压缩上下文",
		timestamp: notice.timestamp,
		source: "client:context_compaction",
	});
}

function completeContextCompactionNotice(sessionId: string): void {
	const notice = activeContextCompactionNotices.get(sessionId) ?? {
		id: `context-compaction-${nextMsgId()}`,
		timestamp: Date.now(),
	};
	agentModel.upsertMessage(sessionId, {
	...
	activeContextCompactionNotices.delete(sessionId);
}

function discardPendingContextCompactionNotice(sessionId: string): void {
	const notice = activeContextCompactionNotices.get(sessionId);
	if (!notice) return;
...
function isRecentLocalSession(sessionId: string, now = Date.now()): boolean {
	const session = agentModel.state.sdkSessions.find(
	...
	return (
		createdAt > 0 &&
		now - createdAt >= 0 &&
		now - createdAt <= RECENT_LOCAL_SESSION_ACCESS_GRACE_MS
	);
}
...
function isPrimaryInternShannonSession(sessionId: string): boolean {
	...
	return (
		agentRegistryModel.resolveSessionAgentId(
			sessionId,
...
function recordPrimaryInternShannonMemoryTimelineEvent(
	sessionId: string,
...
function resolveExposedWorkspacePath(
	...
	return (
		exposeWorkspacePath(storageWorkspace) ||
		exposeWorkspacePath(statusWorkspace) ||
		exposeWorkspacePath(currentCwd)
	);
}

function exposeSessionUpdate(
...
function buildSessionStateFromBackend(
	...
	if ("sessionId" in session) {
		const existing =
			agentModel.state.sessions[session.sessionId || fallbackSessionId];
		return {
			...session,
...
function normalizeRuntimeTimelineEvent(
	event: Record<string, unknown>,
		...
		return {
			...
			elapsedMs:
				typeof event.elapsedMs === "number" && Number.isFinite(event.elapsedMs)
					...
					: undefined,
			activeToolCount:
				typeof event.activeToolCount === "number" &&
				Number.isFinite(event.activeToolCount)
	...
	if (type === "tool_activity") {
		const toolUseId =
			typeof event.toolUseId === "string" && event.toolUseId.trim()
				? event.toolUseId
				: undefined;
		const toolName =
		...
		return {
			id:
				baseId ||
				`tool:${toolUseId || toolName || phase}:${normalizeRuntimeStatus(event.status)}:${timestamp}`,
			kind: "tool",
			status: normalizeRuntimeStatus(event.status),
			phase,
			label: displayLabel,
	...
	if (
		...
		const parentSessionId =
			typeof event.parentSessionId === "string" && event.parentSessionId.trim()
...
function updatePlanningStateFromEvent(
	sessionId: string,
	event: Record<string, unknown>,
): void {
	const next = reducePlanningStateFromEvent(
		agentModel.state.planningStates[sessionId],
		event,
	);
...
function sanitizeAssistantContentBlocks(
	blocks: ContentBlock[],
): ContentBlock[] {
	return blocks
...
function parseToolInputForContentBlock(
	rawInput: string,
...
function mergeFinalBlocksWithStreamingTools(
	...
	streamingSegments:
		| import("@/models/agent.model").StreamingSegment[]
		| undefined,
): ContentBlock[] {
	const streamingToolBlocks =
		streamingToolSegmentsToContentBlocks(streamingSegments);
	...
	const firstTextIndex = finalBlocks.findIndex(
	...
	return [
		...finalBlocks.slice(0, firstTextIndex),
		...missingToolBlocks,
		...finalBlocks.slice(firstTextIndex),
	];
}

function buildSyntheticToolUseId(
	sessionId: string,
	toolName: string,
	seq: number,
): string {
...
function resolveToolUseId(
	...
	const synthetic = buildSyntheticToolUseId(
		sessionId,
		toolName || "tool",
		seq ?? nextStreamSeq(sessionId),
	);
...
function compactCachedToolInput(
	sessionId: string,
	toolUseId: string,
	toolName: string,
	fallbackInput?: string,
): string {
	return compactToolInputForUi(
		toolName,
		getCachedToolInput(sessionId, toolUseId) ?? fallbackInput ?? "",
	);
}
...
function hasUsefulToolInput(
	block: import("@/typings/agent").ContentBlock,
): boolean {
...
function seedToolInputCacheFromBlocks(
	sessionId: string,
	blocks: import("@/typings/agent").ContentBlock[],
): void {
...
function mergePreferRicherToolInputs(
	...
	const existingToolInputs = new Map<
		string,
		import("@/typings/agent").ContentBlock
	>();
			...
			const attach = (url: string) => {
				...
				try {
					...
					socket.on("subscribed", (payload: unknown) => {
						...
						if (isStreamDebugEnabled()) {
							console.log(
								`[Socket.IO] Subscribed to session room ${sessionId}`,
							);
						}
						connected = true;
					...
					socket.on("disconnect", (reason: string) => {
						if (isStreamDebugEnabled()) {
							console.log(
								`[Socket.IO] Disconnected for session ${sessionId}, reason: ${reason}`,
							);
						}
						connectingSessions.delete(sessionId);
						const skipNormalReconnect =
							consumeTransientAccessDisconnect(sessionId);
						if (
							!skipNormalReconnect &&
							!connected &&
							attempt + 1 < urls.length
						) {
							attempt += 1;
							socket.close();
							attach(urls[attempt]);
					...
					socket.on("connect_error", (err: { message: string }) => {
						console.warn(
							`[Socket.IO] Connection error for session ${sessionId}:`,
							err.message,
						);
						connected = false;
					});
					...
					socket.on("connect_timeout", () => {
						console.warn(
							`[Socket.IO] Connection timeout for session ${sessionId}`,
						);
					});
					...
					socket.on("message", (data: unknown) => {
						try {
							...
							if (isStreamDebugEnabled()) {
								console.log(
									`[Socket.IO:msg] sessionId=${sessionId} type=${msg.type}`,
									msg,
								);
							}
					...
					socket.on("exception", (data: unknown) => {
						console.error(
							`[Socket.IO] Exception for session ${sessionId}:`,
							data,
						);
					...
					socket.on("tool_confirmation_request", (data: unknown) => {
						try {
							const request = normalizeToolConfirmationSocketPayload(
								data,
								sessionId,
								nextMsgId(),
							);
							if (!request) {
								console.warn(
									"[HITL] Ignoring malformed tool confirmation request",
									data,
								);
								return;
							}
							if (request.sessionId !== sessionId) {
								console.warn(
									"[HITL] Ignoring tool confirmation request for another session",
									{
							...
							console.log(
								`[HITL] Received tool_confirmation_request for session ${sessionId}`,
								{
									requestId: request.requestId,
							...
							if (autoDecision?.approved && autoDecision.scope) {
								console.log(
									`[HITL] Auto-authorizing tool ${request.toolName} with scope ${autoDecision.scope}`,
								);
								const response = {
									requestId: request.requestId,
									approved: true,
							...
							console.error(
								"[HITL] Error handling tool confirmation request:",
								err,
							);
						}
					});
...
export function disconnectSession(sessionId: string): void {
	...
	diffCache.delete(sessionId);
	pendingUserImages.delete(sessionId);
	sessionStatusRequests.cancelSession(sessionId);
...
export function sendToSession(
	...
	if (isStreamDebugEnabled()) {
		console.log(
			`[WS] sendToSession: socketId=${socket?.id}, connected=${socket?.connected}, msg.type=${msg.type}`,
		);
	}
	if (
		socket &&
...
/** Request a fresh runtime status and resolve only after that session replies. */
export function requestFreshSessionStatus(
	sessionId: string,
	timeoutMs = 3000,
): Promise<boolean> {
	sessionStatusRequestSequence += 1;
	const requestId = `context-${Date.now()}-${sessionStatusRequestSequence}`;
	const pending = sessionStatusRequests.wait(sessionId, requestId, timeoutMs);
	if (
		...
		sessionStatusRequests.settle(sessionId, requestId, false);
	}
	return pending;
}
...
function scheduleTransientAccessRetry(sessionId: string): void {
	...
	const timer = setTimeout(() => {
		reconnectTimers.delete(sessionId);
		if (
			agentModel.state.sessions[sessionId] &&
			isRecentLocalSession(sessionId)
		) {
			connectSession(sessionId);
		}
	}, delay);
...
function handleStreamEventPayload(
	...
	if (
		...
		const rawInput =
			getCachedToolInput(sessionId, resolvedToolUseId) ??
			toolStart.input ??
			existingInput;
	...
	if (eventType === "tool_end") {
		...
		const toolEnd = normalizeStreamToolEndEvent(event, {
			fallbackToolName:
				agentModel.state.activeToolProgress[sessionId]?.toolName,
		});
		if (!toolEnd) return;
		...
		if (before != null || after != null) {
			const sessionDiffs =
				diffCache.get(sessionId) ??
				new Map<
					string,
					{ before?: string; after?: string; filePath?: string }
				>();
			sessionDiffs.set(toolId || toolName, {
				before,
				after,
				filePath: filePath,
			});
	...
	if (eventType === "content_block_start") {
		...
		if (block?.type === "tool_use" && typeof block.name === "string") {
			...
			if (existingActive?.toolUseId && existingActive.toolName === toolName) {
				...
				if (existingId) {
					const existingTp = agentModel.getToolProgress(
						sessionId,
						existingId,
						toolName,
					);
					resolvedToolUseId =
						existingTp?.toolName === toolName
							? existingId
							: resolveToolUseId(sessionId, toolId, toolName, seq);
				} else {
					resolvedToolUseId = resolveToolUseId(
						sessionId,
						toolId,
						toolName,
						seq,
	...
	if (eventType === "content_block_delta") {
		...
		if (
			(delta.type === "text_delta" || delta.type === "output_text_delta") &&
		...
		// input_json_delta inside content_block_delta - tool input streaming
			...
			if (tp) {
				...
				const next = {
					...tp,
					phase: "input_streaming" as const,
					input: appendCachedToolInputDeltaForUi(
						sessionId,
						tp.toolUseId,
						tp.toolName,
						delta.partial_json,
	...
	if (eventType === "btw_answer") {
		...
		const pendingTurn = [...existingTurns]
			.reverse()
	...
	if (eventType === "turn_end") {
		if (isStreamDebugEnabled()) {
			...
			console.log(
				`[stream:${sessionId}] turn_end turn=${turn} totalTokens=${totalTokens}`,
			);
		}
		return;
	}
	...
	if (eventType === "tool_output_delta") {
		...
		const toolUseId = resolveToolUseId(
			sessionId,
			outputDelta.toolUseId,
			toolName,
			seq,
		);
		if (toolName) {
			...
			const tp =
				agentModel.getToolProgress(sessionId, toolUseId, toolName) ??
				agentModel.state.activeToolProgress[sessionId];
			if (tp) {
	...
	if (eventType === "tool_progress") {
		...
		const next = {
			...
			input:
				progress.input ??
				(existing?.toolUseId === toolUseId ? existing.input : undefined),
			output:
				progress.output ??
				(existing?.toolUseId === toolUseId ? existing.output : undefined),
...
function enqueueStreamEvent(
	...
	if (seq < expected) {
		...
		if (seq === 1 || staleness > 20) {
			...
			logStreamDebug(
				sessionId,
				`stale expected reanchor seq=${seq} (staleness=${staleness})`,
			);
			expected = seq;
		} else {
			stats.staleDrops += 1;
			if (stats.staleDrops % 10 === 0) {
				logStreamDebug(
					sessionId,
					`stale drop x${stats.staleDrops} (staleness=${staleness})`,
				);
			}
			return;
		}
	...
	if (seq > expected) {
		...
		if (gap <= REORDER_BUFFER_MAX) {
			...
			logStreamDebug(
				sessionId,
				`buffered seq=${seq} (waiting for expected=${expected})`,
			);
		...
		logStreamDebug(
			sessionId,
			`gap skip expected=${expected} -> seq=${seq} (gap=${gap})`,
		);
	}
...
function flushReorderBuffer(sessionId: string): void {
	...
	logStreamDebug(
		sessionId,
		`flush reorder buffer: ${sortedSeqs.length} events, jumping to seq=${lowestSeq}`,
	);
...
function handleMessage(sessionId: string, msg: BrowserIncomingMessage): void {
	if (isStreamDebugEnabled()) {
		...
		const eventType = (msg as Record<string, unknown>).type as
			| string
			| undefined;
		console.log(
	...
	switch (msg.type) {
		case "session_init": {
			...
			agentRegistryModel.ensureSessionAgent(
				sessionId,
				sessionState.agentId ?? null,
			);
		...
		case "session_status":
			{
				...
				if (Object.keys(sessionPatch).length === 0) {
					sessionStatusRequests.settle(sessionId, msg.requestId, true);
					break;
				}
				agentModel.updateSession(sessionId, sessionPatch);
				if (sessionPatch.runtimeBusy === true) {
					agentModel.setSessionStatus(
						sessionId,
						sessionPatch.runtimePhase === "compacting"
							? "compacting"
							: "running",
					);
				} else if (sessionPatch.runtimeBusy === false) {
					agentModel.setSessionStatus(sessionId, "idle");
				}
				agentRegistryModel.ensureSessionAgent(
					sessionId,
					sessionPatch.agentId ?? null,
				);
				sessionStatusRequests.settle(sessionId, msg.requestId, true);
			}
			break;
		...
		case "assistant": {
			...
			const assistantMessage = normalizeAssistantSocketMessage(
				msg.message,
				nextMsgId(),
			);
			if (!assistantMessage) {
			...
			const chatMsg: AgentChatMessage = {
				...
				knowledgeSources: assistantMessage.knowledgeSources,
				knowledgeSourceProtocolVersion:
					assistantMessage.knowledgeSourceProtocolVersion,
			};
		...
		case "result": {
			...
			const currentRunId = agentModel.state.sessions[sessionId]?.activeRunId;
			const terminalMatches = runtimeTerminalMatches(
				currentRunId,
				resultMessage.runId,
			);
		...
		case "stream_event":
			if (msg.event && typeof msg.event === "object") {
				...
				if (event.type === "context_compacted") {
					completeContextCompactionNotice(sessionId);
					agentModel.setSessionStatus(sessionId, "running");
					agentModel.updateSession(sessionId, {
						isCompacting: false,
						runtimeBusy: true,
				...
				if (runtimeTimelineEvent) {
					if (
						...
						(runtimeTimelineEvent.status === "queued" ||
							runtimeTimelineEvent.phase === "intake")
					) {
		...
		case "cancelled":
			if (
				!runtimeCancellationMatches(
					agentModel.state.sessions[sessionId]?.activeRunId,
					msg.runId,
				)
			) {
				break;
		...
		case "status_change":
			if (msg.status === "compacting") {
				startContextCompactionNotice(sessionId);
				...
				const activeRunId = agentModel.state.sessions[sessionId]?.activeRunId;
				if (!runtimeTerminalMatches(activeRunId, msg.runId)) {
					break;
				}
		...
		case "error":
			...
			agentModel.appendMessage(sessionId, {
				...
				timestamp: Date.now(),
				source: normalizeSocketOptionalText(msg.code)
					? `error:${normalizeSocketText(msg.code)}`
					: "error",
			});
		...
		case "message_history": {
			...
			logToolDebug(sessionId, "message history tool blocks", {
				items: historyMessages
					...
					.flatMap((m) =>
						normalizeHistoryAssistantMessageContentBlocks(
							normalizeHistoryRecord(m.message),
						)
							.filter((b) => b.type === "tool_use")
							.map((b) => ({
								type: b.type,
			...
			for (const historyMessage of historyMessages) {
				if (
					...
					updatePlanningStateFromEvent(
						sessionId,
						historyMessage.event as Record<string, unknown>,
					);
				}
			}
			if (
		...
		case "command_response":
			...
			if (
				normalizeSocketText(msg.command) === "/clear" &&
				normalizeSocketBoolean(msg.stateChanged)
			) {
...
function isSessionAccessError(message: unknown): boolean {
	...
	return /session not found or access denied|kernel session not found/i.test(
		message,
	);
}
...
function convertHistoryMessages(
	...
	const existingAssistantBlocks = new Map(
		existingMessages.flatMap((msg) =>
			msg.role === "assistant" && msg.contentBlocks?.length
				? [[msg.id, msg.contentBlocks] as const]
				: [],
		),
	);
	for (const [messageId, blocks] of Object.entries(
		agentModel.getPersistedAssistantBlocks(sessionId),
	)) {
	...
	for (const msg of normalizeMessageHistoryItems(messages)) {
		if (msg.type === "assistant") {
			...
			const durationMs = normalizeHistoryFiniteNumber(
			...
			const mergedBlocks = mergePreferRicherToolInputs(
				visibleContentBlocks,
				existingAssistantBlocks.get(messageId),
			);
			const finalBlocks = enrichToolUseInput(
				sessionId,
				sanitizeAssistantContentBlocks(mergedBlocks),
			);
			seedToolInputCacheFromBlocks(sessionId, finalBlocks);
...
function mergeBypassConversations(
	...
	for (const turn of incoming) {
		const duplicate = merged.some(
			(existingTurn) =>
				existingTurn.question.trim() === turn.question.trim() &&
				existingTurn.answer.trim() === turn.answer.trim(),
		);
		if (!duplicate) {
			merged.push(turn);
		}
...
function extractBypassConversations(
	messages: unknown,
): import("@/models/agent.model").BypassConversationTurn[] {
