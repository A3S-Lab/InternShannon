...
const RESULT_NUMBER_FIELDS = [
	...
	["contextUsedTokens", "contextUsedTokens", "context_used_tokens"],
	["contextMaxTokens", "contextMaxTokens", "context_max_tokens"],
...
export function normalizeResultMessageData(
	value: unknown,
	...
	const lastRunDurationMs = normalizeNumberField(value, [
		"durationMs",
		"duration_ms",
	]);
	const lastRunTotalTokens = normalizeNumberField(value, [
		"totalTokens",
		"total_tokens",
	]);
	const lastRunToolCalls = normalizeNumberField(value, [
		"toolCalls",
		"tool_calls",
	]);
	const lastRunActiveToolCount = normalizeNumberField(value, [
		"activeToolCount",
		"active_tool_count",
	]);
	const lastRunOpenPlanTasks = normalizeNumberField(value, [
		"openPlanTasks",
		"open_plan_tasks",
	]);
	if (lastRunDurationMs !== undefined)
		sessionPatch.lastRunDurationMs = lastRunDurationMs;
	if (lastRunTotalTokens !== undefined)
		sessionPatch.lastRunTotalTokens = lastRunTotalTokens;
	if (lastRunToolCalls !== undefined)
		sessionPatch.lastRunToolCalls = lastRunToolCalls;
	if (lastRunActiveToolCount !== undefined)
		sessionPatch.lastRunActiveToolCount = lastRunActiveToolCount;
	if (lastRunOpenPlanTasks !== undefined)
		sessionPatch.lastRunOpenPlanTasks = lastRunOpenPlanTasks;

	const runStatus = normalizeRunStatus(
		value.status ?? value.runStatus ?? value.run_status,
	);
	if (runStatus) sessionPatch.lastRunStatus = runStatus;
	const stopReason = normalizeOptionalString(
		value.stopReason ?? value.stop_reason,
	);
	...
	const contextUsagePendingRefresh = normalizeOptionalBoolean(
		value.contextUsagePendingRefresh ?? value.context_usage_pending_refresh,
	);
	if (contextUsagePendingRefresh !== undefined) {
		sessionPatch.contextUsagePendingRefresh = contextUsagePendingRefresh;
	}
	if (
		sessionPatch.contextUsedTokens !== undefined &&
		contextUsagePendingRefresh !== true
	) {
		// A turn_end prompt measurement is authoritative. Do not retain an
		// earlier cold-start/post-compaction "estimated" badge after it arrives.
		sessionPatch.contextUsageEstimated = false;
	}
	...
	const normalized: NormalizedResultMessage = {
		...
		errorContent:
			normalizeFirstResultText(value.result, value.error, value.message) ||
			"An error occurred",
	};
	const runId = normalizeOptionalString(value.runId ?? value.run_id);
	if (runId !== undefined) normalized.runId = runId;
	if (
		runStatus === "incomplete" &&
		stopReason === "sdk_stream_ended_without_stop_reason" &&
		lastRunOpenPlanTasks === 0 &&
		(lastRunActiveToolCount === 0 ||
			(lastRunActiveToolCount === undefined && lastRunToolCalls === 0))
	) {
...
function normalizeRunStatus(
	value: unknown,
): AgentSessionState["lastRunStatus"] | undefined {
	if (typeof value !== "string") return undefined;
	const normalized = value.trim().toLowerCase();
	if (
