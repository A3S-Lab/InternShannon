		...
		),
		{
			...
			contentBlocks: [
				{ type: "text", text: "visible answer" },
				{
					...
					id: "tool-3",
					name: "tool",
					input: { __display: "path:README.md" },
				},
				{
					type: "tool_result",
					toolUseId: "tool-3",
...
test("ignores malformed live assistant messages", () => {
	assert.equal(
		normalizeAssistantSocketMessage(null, "assistant-fallback"),
		null,
	);
	assert.equal(
		normalizeAssistantSocketMessage(
			"legacy answer without message object",
			"assistant-fallback",
		),
		null,
	);
...
test("keeps verified structured sources on live assistant messages", () => {
	const resource = "asset://asset-1/raw/sources/orders.csv";
	const normalized = normalizeAssistantSocketMessage(
		{
	...
	assert.equal(normalized?.knowledgeSourceProtocolVersion, 1);
	assert.equal(normalized?.knowledgeSources?.[0]?.resource, resource);
});
