...
test("normalizes the run id used to reject stale terminal events", () => {
	assert.equal(
		normalizeResultMessageData({ run_id: " run-42 " }).runId,
		"run-42",
	);
});
...
test("normalizes snake_case result metrics before InternShannon updates session counters", () => {
	assert.deepEqual(
		normalizeResultMessageData({
			...
			context_used_tokens: "122550",
			context_max_tokens: "258000",
			context_usage_pending_refresh: "false",
			input_tokens: "1000",
			output_tokens: "250",
		...
		}),
		{
			sessionPatch: {
				...
				contextUsedTokens: 122550,
				contextMaxTokens: 258000,
...
test("preserves string result error content", () => {
	assert.deepEqual(
		normalizeResultMessageData({ is_error: true, result: "sidecar failed" }),
		{
			sessionPatch: {},
			isError: true,
			errorContent: "sidecar failed",
